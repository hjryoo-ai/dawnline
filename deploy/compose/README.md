...
**같은 볼륨에서 `make demo` 를 거듭 돌리면 다섯째 실행 무렵 첫 줄에서 멈춘다**(「밀림 소진 캠프 · 티어」). 데모는 실행마다 웨이브
하나를 컷오프 전에 닫고(phase6-demo — 운영자 조기 마감), 같은 캠프 · 티어의 웨이브 넷이 연달아 닫히면 그 뒤의 주문은 받을 웨이브가
없다(`MAX_PUSHES_EXCEEDED`, ADR-063). 볼륨을 새로 하거나 닫힌 웨이브의 컷오프가 지난 뒤에 돌린다.
