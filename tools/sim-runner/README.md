...
**스캔 본문이 그 stop 의 `orderIds` 를 싣는다** (2026-09-23, ADR-047 결정 1). 경로의
`{routeId}`·`{stopSeq}` 는 tracking 에서 확인용이고 대상은 송장으로 찾는다 — 개정이 바꾸는
것은 번호이지 송장이 아니므로, 재계획 직후에 옛 번호로 찍어도 맞는 배송에 적용된다. 그래서
「끝낸 주문」을 세는 축과 「보내는 요청」의 축이 처음으로 같아졌다: `TripProgress` 는 개정에서
`seq` 로 되찾지 않고 보낸 스캔이 든 목록을 그대로 쓴다. `DEPARTED_CAMP` 만 비운다 — 캠프
출발은 라우트의 사건이고, 실어 보내면 tracking 이 400 으로 답한다.
