...
public class ProblemDetailsAdvice extends ResponseEntityExceptionHandler {
     ...
     * 재시도해도 같은 답이 온다. 표를 비워 두는 이유는 order-service 와 같은 형태를 유지해
     * 나중에 생기는 값이 <em>어디에</em> 들어가야 하는지가 분명하기 때문이다.
     */
    private static final Map<String, Integer> RETRY_AFTER_SECONDS = Map.of();
