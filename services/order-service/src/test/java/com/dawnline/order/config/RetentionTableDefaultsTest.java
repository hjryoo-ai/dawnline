...
/**
 * §7.1 보존 표 ↔ 이 모듈의 설정 기본값 (order-service, ADR-058 결정 7).
 *
...
@DisplayName("§7.1 보존 표 ↔ 설정 기본값 (order-service)")
class RetentionTableDefaultsTest {

    /** 이 모듈의 설정 접두사. */
    private static final String PREFIX = "dawnline.order.";

    private static final OrderProperties DEFAULTS = new Binder(new MapConfigurationPropertySource(Map.of()))
    ...
    private static final Map<String, Duration> PERIODS = Map.of(
            "dawnline.order.idempotency.retention-days", DEFAULTS.idempotency().retention());
