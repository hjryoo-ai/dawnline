...
/**
 * §7.1 보존 표 ↔ 이 모듈의 설정 기본값 (ops-api, ADR-058 결정 7).
 *
...
@DisplayName("§7.1 보존 표 ↔ 설정 기본값 (ops-api)")
class RetentionTableDefaultsTest {

    /** 이 모듈의 설정 접두사. */
    private static final String PREFIX = "dawnline.ops.";

    private static final OpsRetentionProperties DEFAULTS = new Binder(new MapConfigurationPropertySource(Map.of()))
    ...
    private static final Map<String, Duration> PERIODS = Map.of(
            "dawnline.ops.retention.orders", DEFAULTS.orders(),
            "dawnline.ops.retention.orders-cap", DEFAULTS.ordersCap(),
            "dawnline.ops.retention.routes", DEFAULTS.routes(),
            "dawnline.ops.retention.waves", DEFAULTS.waves());
    ...
    void 보존_레코드의_기간_칸은_전부_위_목록에_있다() {
        ...
        assertThat(PERIODS.keySet()).containsAll(RetentionTable.durationKeys(OpsRetentionProperties.class, "dawnline.ops.retention"));
    }
}
