...
/**
 * ops-api 아키텍처 경계 테스트 (DESIGN.md §13, CLAUDE.md 불변규칙 3·4·5).
 *
 ...
 */
@AnalyzeClasses(importOptions = ImportOption.DoNotIncludeTests.class, packages = "com.dawnline.ops")
class ArchitectureTest {

    /** {@link HexagonalArchitectureRules#SERVICES} 의 서비스 식별자. */
    private static final String SERVICE = "ops";

    /**
     * {@code @AnalyzeClasses} 에는 컴파일 상수만 넣을 수 있어 리터럴로 적고,
     * {@link HexagonalArchitectureRules#packageOf(String)} 와 같은지 아래에서 검증한다.
     */
    private static final String ANALYZED_PACKAGE = "com.dawnline.ops";
    ...
    static void 헥사고날_규칙을_모두_지킨다(JavaClasses classes) {
        ...
        assertThat(rules).as("DESIGN.md §13 의 서비스 대상 ArchUnit 규칙 10개 (규칙 10 은 libs/common 에 건다, 규칙 12 는 ops 가 빠진다 — HTTP_CLIENT_OWNERS)").hasSize(10);
