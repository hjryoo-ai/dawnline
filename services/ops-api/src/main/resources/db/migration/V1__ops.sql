...
--
-- 인덱스는 §5.5 에 적힌 것만 만든다 (CLAUDE.md 불변규칙 11). PK 외에는 rm_orders 의 둘뿐이다.
-- =============================================================================
...
-- 개수 칸을 다시 세는 두 질의가 쓴다(ADR-051 결정 4) — §5.5 명시분.
-- 측정: docs/benchmarks/phase6-rm-orders-aggregate-index.md. rm_orders 는 보존 정책이 없어 피크일
-- 15만 행씩 쌓이고, 인덱스 없이는 1일치에서도 라우트 재집계가 9.2 ms(배송 673 건/초면 초당 6초의
-- DB 시간)다. 인덱스로 0.19–0.25 ms, 30일치에서도 평평하다.
CREATE INDEX ix_rmo_route ON rm_orders (route_id);
CREATE INDEX ix_rmo_wave  ON rm_orders (wave_id);
