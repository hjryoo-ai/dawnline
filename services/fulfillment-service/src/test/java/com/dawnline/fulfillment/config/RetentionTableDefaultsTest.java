...
/**
 * §7.1 보존 표 ↔ 이 모듈의 설정 기본값 (fulfillment-service, ADR-058 결정 7).
 *
...
@DisplayName("§7.1 보존 표 ↔ 설정 기본값 (fulfillment-service)")
class RetentionTableDefaultsTest {

    /** 이 모듈의 설정 접두사. */
    private static final String PREFIX = "dawnline.fulfillment.";

    private static final FulfillmentProperties DEFAULTS = new Binder(new MapConfigurationPropertySource(Map.of()))
            .bindOrCreate("dawnline.fulfillment", FulfillmentProperties.class);

    /** 이 모듈의 보존 기간 — 설정 키 → 기본값. */
    private static final Map<String, Duration> PERIODS = Map.of(
            "dawnline.fulfillment.retention.orders", DEFAULTS.retention().orders(),
            "dawnline.fulfillment.retention.waves", DEFAULTS.retention().waves());
    ...
    void 보존_레코드의_기간_칸은_전부_위_목록에_있다() {
        ...
        assertThat(PERIODS.keySet()).containsAll(RetentionTable.durationKeys(FulfillmentProperties.Retention.class, "dawnline.fulfillment.retention"));
    }
}
