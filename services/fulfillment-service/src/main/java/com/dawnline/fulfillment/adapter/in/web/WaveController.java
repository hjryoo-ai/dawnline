    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "409",
                    description = "`wave-not-open` — 이미 닫혀 있다. 확장 멤버 `currentState`·`closeCause`·`closedAt` 이 "
                            ...
                            + "`MANUAL` 이면 앞의 요청이 적용된 것이다. 또는 `not-next-wave` — 같은 캠프 · 티어에 더 이른 컷오프의 "
                            + "열린 웨이브가 있다(ADR-054 후속). 확장 멤버 `earlierWaveId`·`earlierCutoffAt` 이 먼저 닫아야 할 웨이브다. "
                            + "대상이 이미 닫혀 있으면 이 판정은 하지 않는다 — 다시 누른 사람은 늘 `wave-not-open` 을 받는다",
                    content = @Content(schema = @Schema(implementation = ProblemDetail.class)))})
