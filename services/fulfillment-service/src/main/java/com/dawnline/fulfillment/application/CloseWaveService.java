...
public class CloseWaveService implements CloseWaveUseCase {
     ...
     */
    public CloseWaveService(WaveClosing closing, PlatformTransactionManager transactionManager,
    ...
    private WaveView closeInScope(UUID waveId, String reason) {
        WaveClosing.Outcome outcome = transactions.execute(status -> closing.closeIfOpen(waveId, WaveCloseCause.MANUAL));
