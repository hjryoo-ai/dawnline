...
- ~~**`route:{id}:progress` 는 불변규칙 7 의 대상이다.**~~ **그 키는 지웠다** (2026-09-23,
  Phase 6-0c — [ADR-048](ADR-048-replan-reads-its-own-db.md) 재검토 지점 4). 여기 적힌 「첫
  소비자는 §6.8 의 부분 재계획(Phase 5-3)」이 **오지 않았고**, 5-5 가 만든 것은 쓰는 쪽과
  폴백뿐이었다. 읽는 쪽 없는 쓰기는 §7.2 에 행이 있다는 이유로 유지되지 않는다 —
  `GET /routes/{routeId}` 가 stop 마다 살아 있는 상태를 이미 돌려주므로 세 칸은 그 응답에서
  나온다. **이 줄을 지우지 않는 이유**: 여기가 「첫 소비자」를 이름으로 적어 둔 자리이고,
  그 예정이 빗나갔다는 것 자체가 다음번에 같은 형태를 세울 때의 근거다.
- **`route_stop_orders (order_id)` 인덱스를 하나 넣는다** (`V9`, §5.3). 질의는 새것이 아니지만
  ...
  **→ 2026-09-25 구현했다**([ADR-059](ADR-059-dispatch-retention-is-per-plan.md)): 계획 계열이 계획 단위로 90일에 지워진다 — `route_stop_orders` 는
  91일치 약 1,365만 행이 평상시 최대이고, 「계속 나빠진다」는 상한이 있는 값이 됐다. 인덱스 판단은 그대로다.
- **재검토 지점 ① — 닫혔다** (2026-09-23, Phase 5-3). 결정 3 의 근거는 「근거: 추정」이었다:
  ...
  `ReplanIT.옮겨간_주문의_뒤늦은_완료는_옮겨간_라우트에_적용된다` 가 그 자리를 실물로 본다:
  재계획이 주문을 B 로 옮긴 **뒤에** 기사가 옛 라우트 A 의 번호로 찍은 완료를 보내면, 그 사실은
  B 의 stop 에 적용되고 `dawnline_status_after_relocate_total` 이 오른다. 결정 3 의 근거는
  이제 **관측(재현됨)** 이다. 남은 관찰 지점은 그 카운터의 <em>폭</em>이고, 그것은 이상이 아니라
  §6.8 경합 창의 크기다.
