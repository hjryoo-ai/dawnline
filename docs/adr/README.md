...
| 061 | **끝나지 않은 일에는 창이 없다** — 라우트의 완료는 쓰기 때 다시 센다(`rm_routes.completed_at`, 재집계 칸 · 사실에서 온 값) · `order.cancelled` 도 다시 센다 · `in_progress` · `unknown` 은 창 없음, `assigned` · `completed` 는 KPI 창 · 질의는 `rm_routes` 만(5–8 ms, 인덱스 없음) | ✅ Accepted (2026-09-25) | [ADR-061](ADR-061-unfinished-work-has-no-window.md) |
