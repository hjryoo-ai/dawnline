...
- **`processed_events` 가 서비스마다 무한히 증가한다.** 보존 기간은 "같은 이벤트가 재전달될 수 있는 최대 시간"
  ...
  **[Phase 1 확정] 14일.** 재전달 창의 상한은 본 토픽 보존 7일이고 14일은 그 2배 여유다. DLQ 보존 30일은
  이 창과 무관하다 — DLQ 에 들어간 이벤트는 처리 트랜잭션이 롤백된 것이라 `processed_events` 에 성공 기록이
  없기 때문이다. 근거와 그 논거가 의존하는 전제는 DESIGN §4.4 에 적었다.
