...
| 결정일 | 2026-09-25 |
| 관련 문서 | `docs/DESIGN.md` §5.5 「`rm_routes.completed_at` · `live_count`」 · §9.1 `dawnline_routes` · `docs/benchmarks/phase7-route-progress-count.md` · `docs/IMPLEMENTATION_PLAN.md` 7-1 A6 |
...
### 1. 「완료」는 사건 시점에 그 라우트만 다시 세어 적는다 — `rm_routes.completed_at` · `live_count`
...
- **`live_count`**: 그 라우트의 비취소 주문 수. **0 이면 그 라우트는 void 다**(결정 3 의 다섯째 값) — 할 일이 없는
  라우트다. 재계획이 주문을 전부 옮겼거나 전부 취소됐다.
- **`completed_at`**: 비취소 주문이 있고 그중 결과 없는 것이 **남지 않았으면** 주문들의 마지막 결과 시각
  (`max(COALESCE(delivered_at, failed_at))`), 아니면 `NULL`. **void 에는 시각이 없다** — 일어나지 않은 완료에 시각을
  만들지 않는다(대안 F).
- **값은 사실에서 온다 — 처리 시각이 아니다.** 입력은 전부 `rm_orders` 의 사실 칸이고, 소비자의 `now()` 를 쓰지 않는다. 그래서 순서와 무관하다 — `ProjectionShuffleIT` 가 이 칸을 다른 집계 칸과 같이 본다.
...
| 사실 | 바뀌는 입력 | 다시 세는 라우트 |
|---|---|---|
| `route.assigned` | 라우트의 주문 집합 | 그 라우트와, 옮겨 온 주문이 떠나온 라우트(기존) |
| `delivery.status`(결과) | 주문의 결과 | 주문의 **지금 계획상** 라우트(기존, ADR-047) |
| `order.cancelled` | 주문이 「남은 주문」과 `live_count` 에서 빠진다 | 주문의 지금 라우트 — **새로 더한다** |

`order.cancelled` 가 앞에 오고 `route.assigned` 가 뒤에 오면, 뒤의 재집계가 그 취소를 본다 — 어느 순서든 마지막으로 도착한
사실이 전체를 다시 센다.

### 3. 읽기 — 끝나지 않은 것은 창 없이, 끝난 것과 할 일이 없는 것은 창 안에서

`dawnline_routes` 의 다섯 값(판정 순서대로):

- `unknown` — 계획이 오지 않았다(`revision IS NULL OR status IS NULL`). **창 없음.**
- `void` — 비취소 주문이 없다(`live_count = 0`). 출발 여부와 무관하다. 계획 출발이 KPI 창의 첫 버킷 이후.
- `assigned` — `status = 'ASSIGNED'`, 비취소 주문이 있다. **창 없음.**
- `in_progress` — `status = 'DEPARTED' AND completed_at IS NULL`, void 가 아니다. **창 없음.**
- `completed` — `status = 'DEPARTED' AND completed_at IS NOT NULL`. 계획 출발이 KPI 창의 첫 버킷 이후.

앞의 셋(`unknown` · `assigned` · `in_progress`)은 끝나지 않은 일이라 창이 없고, 뒤의 둘은 끝났거나 할 일이 없는 것이라
수를 묶는 창이 맞다. 질의는 `rm_routes` 하나만 읽는다 — 범위 술어는
`(completed_at IS NULL AND live_count IS DISTINCT FROM 0) OR planned_departure >= ?` 한 줄이다.

**void 는 완료가 아니다.** 이 결정의 첫 안(같은 PR 의 첫 판)은 빈 라우트 · 전부 취소된 라우트의 `completed_at` 에 계획
출발 시각을 적었다 — 출발한 빈 라우트가 영원히 `in_progress` 로 남지 않게 하려는 것이었다. 그 답은 가짜 완료가 아니라
다섯째 상태다(대안 F). **부재를 값으로 읽지 않는 것(ADR-051)과 값을 만들어 내지 않는 것은 같은 규칙의 양면이다.**
그리고 `completed` 에 한 번도 돌지 않은 라우트가 섞이는 것도 사라진다.

**`assigned` 의 창도 걷었다.** 첫 판은 `assigned` 를 창 안에서만 셌다 — 재계획이 비운 라우트가 영원히 `assigned` 로 남지
않게 하려는 것이었고, 그 대가로 계획 출발을 한참 넘기고도 떠나지 않은 라우트가 창 밖으로 빠졌다. 비운 라우트는 이제
void 이므로 그 대가를 치를 이유가 없다. 떠나지 않은 라우트는 끝나지 않은 일이다.
...
운영 크기(`rm_routes` 112,567, 백필 뒤 39 MB)에서 질의는 `rm_routes` 순차 스캔 한 번이고 **7.1–10.7 ms** 다(첫 판의 네 값
문장은 5.2–8.2 ms — 같은 스캔이고 차이는 칸 둘을 더한 백필이 늘린 페이지와 측정 간 편차다). 비용 추정이 7,174 라 JIT 도
켜지지 않는다. 끝나지 않은 라우트의 부분 인덱스(`… WHERE completed_at IS NULL AND status IS DISTINCT FROM 'ASSIGNED'`,
16 kB)를 만들어 두 형태로 쟀다(첫 판).
...
참·거짓이 갈린다. 계획 선택은 규모에 따라 갈리지만 문장 구조는 규모와 무관하다 — 가드를 둘 중 어디에 걸 수 있는지를 먼저
묻는다(`docs/DESIGN.md` §13 축 4 의 대응).

### 5. 쓰기의 대가는 재지 않을 만큼 작다 — 쟀다

재집계는 이미 결과 사실마다 한 번 돈다. 칸 둘을 세던 두 부분 질의가 넷을 세는 한 부분 질의가 됐다 — 라우트 하나(주문 121)
에서 **0.08–0.21 ms**, 이전 형태 0.07–0.15 ms. 같은 범위다. 백필(V8)은 운영 크기에서 1.4 초(캐시가 찬 뒤 — 적재 직후
첫 실행은 6.7 초)다.
...
| **C** — 남은 주문 수(`pending_count`)를 칸으로 둔다 | 결과 사실마다 값이 바뀌어 그 칸을 술어로 쓰는 인덱스가 생기면 HOT 갱신을 매번 잃는다. `completed_at` 은 라우트당 한 번 `NULL → 값` 으로 바뀐다. 그리고 완료 시각은 지도와 런북이 쓸 사실이다. `live_count`(결정 1)는 이 칸이 아니다 — 결과가 아니라 배정 · 취소에서만 바뀐다 |
...
| **F** — 빈 라우트 · 전부 취소된 라우트의 `completed_at` 을 계획 출발 시각으로 적는다(이 PR 의 첫 판) | 일어나지 않은 완료에 시각을 만든다. 「출발한 빈 라우트가 영원히 `in_progress` 로 남는다」의 답은 가짜 완료가 아니라 다섯째 상태 `void` 다. 그리고 `completed` 에 한 번도 돌지 않은 라우트가 섞인다(결정 3). 사용자 리뷰로 기각 |

## 결과

- `rm_routes` 에 칸 둘(`live_count` · `completed_at`, V8). 핸들러가 쓰지 않는 집계 칸이라 `RouteColumn` 에 없다.
- `dawnline_routes{status}` 의 닫힌 값이 넷에서 다섯이 된다(`void`). 시계열은 캠프 수 × 5.
- `order.cancelled` 핸들러가 라우트 행을 잠근다 — 잠금 순서(주문 → 라우트)는 그대로다.
- `dawnline_routes{status=~"in_progress|assigned"}` 가 창 밖의 끝나지 않은 라우트를 센다. 대시보드의 패널 식은 바뀌지 않는다
  (`sum by (camp, status)` 라서 `void` 도 그대로 나타난다).
...
- ~~출발 전(`assigned`)도 창이 있다 — 출발 지연이 운영 절차에 들어오면 다시 본다.~~ **닫힘(2026-09-25)**: 그 창은 재계획이
  비운 라우트를 `assigned` 에서 치우려는 것이었고, 비운 라우트는 이제 `void` 다(결정 3). `assigned` 는 창이 없다.
