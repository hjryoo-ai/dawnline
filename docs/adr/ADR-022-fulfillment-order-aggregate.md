...
**[후속 정정 — Phase 2-5]** [ADR-025](ADR-025-wave-admission-share-lock.md) 가 `order_count` 를
편입마다 증감하지 않고 **마감 시 한 번 세는 것**으로 바꾸면서, 위 표의 **두 번째 행이 사라진다.**

| 순서 | 웨이브 상태 | 처리 (정정) |
|---|---|---|
| **취소 후착** | 상태와 무관 | 상태만 `CANCELLED`. 카운트를 건드리는 분기가 없다 |

마감 집계가 `status='PLANNED'` 만 세므로, 마감 <em>전</em> 취소는 자동으로 빠지고 마감 <em>후</em>
취소는 이미 나간 숫자를 바꾸지 않는다. 위 세 번째 행이 지키려던 성질("이미 나간 이벤트가
진실이다")이 <strong>분기가 아니라 구조로</strong> 보장된다 — 그리고 카운터 드리프트라는 다른
위험도 함께 사라진다.
...
| **취소 후착 시 마감된 웨이브의 `order_count` 도 줄인다** | 이미 발행된 `wave.closed` 의 `orderCount` 와 DB 가 어긋난다. "그때 몇 건이었나" 에 답이 둘이 된다 (ADR-025 이후에는 이 분기 자체가 없다) |
