...
- **보존 정책이 없다.** `route_revisions` 는 `shipments` 와 같이 늘어나고, §7.1 의 보존 표에
  ...
  **→ 2026-09-25 닫았다**([ADR-058](ADR-058-shipment-and-read-model-retention.md), 7-0b): `shipments` 종결 30일,
  이 표는 90일이고 참조하는 `shipments` 가 없을 때만 지운다 — 같은 자리(§7.1 보존 표)에서 함께 정했다. 이 표의
  개정 비교가 `shipments` 30일(DLQ 와 등호)의 둘째 방어다.
