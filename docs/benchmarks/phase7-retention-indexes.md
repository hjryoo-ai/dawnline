...
- `TrackingRetentionIndexIT`와 `ReadModelRetentionIndexIT`
  ...
  - 어댑터의 상수 그대로 문장을 준비하고, 삭제는 custom과 generic **두 계획 모두**가 인덱스를 타는지 봅니다.
  - **걸린 행 셈은 custom 계획만 봅니다.**
    - 하루 한 번 부르는 문장이라 도는 것은 값을 아는 계획입니다. 서버는 처음 다섯 번을 custom으로 짜고, 그 뒤에도
      generic이 더 쌀 때만 바꿉니다.
    - 첫 CI(#65)에서 15만 행 규모의 generic 계획이 순차 스캔을 골랐습니다. 픽스처에 「결과 없음」 행이 없어 선택도가
      0으로 잡혔고, 같은 DB의 다른 행이 통계를 바꿨기 때문입니다.
