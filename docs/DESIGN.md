...
CREATE TABLE rm_routes (route_id UUID PK, plan_id UUID, camp_id UUID, vehicle_id UUID, driver_id UUID,
  ...
  updated_at TIMESTAMPTZ NOT NULL,                    -- 보존의 나이 (V6, 2026-09-25, ADR-058)
  completed_at TIMESTAMPTZ);                          -- 집계 — 남은 주문이 없을 때의 마지막 결과 시각 (V8, 2026-09-25, ADR-061)
...
- **`rm_routes.completed_at`** (2026-09-25, [ADR-061](adr/ADR-061-unfinished-work-has-no-window.md)). 「완료」는
  ...
  쓰지 않는다, ADR-051 결정 4). 값: 그 라우트에 결과 없는 비취소 주문이 없으면 주문들의 마지막 결과 시각, 있으면 `NULL`,
  결과가 하나도 없는데 남은 주문도 없으면(빈 라우트 · 전부 취소) 계획 출발. 전부 사실이라 처리 순서를 타지 않는다.
  다시 세는 사실은 셋이다 — `route.assigned`(주문 집합 · 계획 출발) · `delivery.status`(결과) · `order.cancelled`(남은
...
| `dawnline_routes` | gauge | **ops-api** | camp, status(assigned/in_progress/completed/unknown) — `rm_routes` 를 **진행**으로 센 집계, KPI 와 **같은 1분 갱신**에서(한 갱신이 함께 성공하고 함께 실패한다 — 알림은 `dawnline_kpi_refresh_age_seconds` 하나다). `rm_routes.status` 는 둘(`ASSIGNED` · `DEPARTED`)뿐이라 완료는 판정이다 — 그 판정은 **쓰기 때** 한다(`rm_routes.completed_at`, §5.5 · [ADR-061](adr/ADR-061-unfinished-work-has-no-window.md)): `assigned` 출발 전 · `in_progress` `status = 'DEPARTED' AND completed_at IS NULL` — 출발했고 결과 없는 주문(취소 제외)이 남았다 · `completed` 출발했고 남은 주문이 없다(주문 수는 stop 수가 아니라서 `completed_count + failed_count` 를 `stop_count` 와 견주지 않는다) · `unknown` 계획(`route.assigned`)이 아직 오지 않은 행 — 판정할 기준(그 라우트의 주문)이 없다. **부재는 값이 아니지만 부재의 수는 값이다.** 그 행의 캠프를 모르면 `camp="unknown"` 이다(`delivery.status` 는 캠프를 싣지 않는다). **범위 — 끝나지 않은 일에는 창이 없다**: `in_progress` · `unknown` 은 창 없이 전부, `assigned` · `completed` 는 계획 출발이 정시율 창의 첫 버킷 이후인 것만(수를 묶는 창). 출발한 지 30시간 된 라우트가 아직 끝나지 않았다면 운영자가 가장 먼저 볼 라우트다 — `dawnline_fulfillment_orders_stuck` 에 상한이 없는 것과 같은 이유다. 질의는 `rm_routes` 하나만 읽는다(보존 90일 11만 행에서 순차 스캔 5–8 ms, 인덱스 없음 — [측정](benchmarks/phase7-route-progress-count.md)). 처음(7-1, #68)에는 읽기가 라우트마다 `rm_orders` 를 찾았고 그 탐색을 묶으려고 전부에 창을 걸었다 — 창 밖의 진행 중 라우트가 빠졌다(운영 크기에서 471 중 55). **라우트 단위가 아니라 집계다**: `routeId` 는 열린 라벨이고 피크일 1,250 개씩 새 값이 생겨 시계열 수가 끝없이 는다 — Prometheus 의 비용은 시계열 수이고, 한 번 생긴 시계열은 보존 기간 내내 남는다. 이 행의 시계열은 캠프 수 × 4 가 전부다. 라우트 단위 진행은 ops-web 지도(`GET /waves/{waveId}/routes`)의 몫이다. 갱신 실패 중에는 `NaN` |
...
| 061 | **끝나지 않은 일에는 창이 없다 — 라우트의 완료는 쓰기 때 다시 센다** — `rm_routes.completed_at`(V8)은 `completed_count` · `failed_count` 와 같은 재집계가 쓰는 집계 칸(ADR-051 결정 4 의 행 단위), 값은 사실에서(마지막 결과 시각, 빈 라우트는 계획 출발) · 다시 세는 사실은 `route.assigned` · `delivery.status` · **`order.cancelled`(새로)** · `dawnline_routes` 의 `in_progress`(`status = 'DEPARTED' AND completed_at IS NULL`) · `unknown` 은 창 없음, `assigned` · `completed` 는 KPI 창 · 질의는 `rm_routes` 만 읽는다(운영 크기 5–8 ms, **인덱스 없음** — 부분 인덱스는 창 쪽 순차 스캔 때문에 값을 하지 않았다) · #68 의 창이 창 밖 진행 중 55 라우트를 뺐고, 창만 걷으면 매분 0.9 초 · 1.7 GB 였다 | [ADR-061](adr/ADR-061-unfinished-work-has-no-window.md) |
