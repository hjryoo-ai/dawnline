...
| 060 | **메트릭은 표에서 온다** — §9.1 의 행마다 카탈로그 항목 하나(`DawnlineMetrics`: Prometheus 이름 · 타입 · Micrometer 이름 · **라벨 집합** — 닫힘/열림), 표 ↔ 카탈로그 양방향 대조 · 등록은 `DawnlineMeters` 한 곳(타입 · 라벨 키 · 닫힌 값을 등록 때 대조, `histogram` 은 버킷, **게이지는 강한 참조** — 약한 참조의 `NaN` 이 「모름」 검사를 대상 없이 통과시켰다, 근거: 관측(재현됨)), 우회는 ArchUnit 규칙 11 · 알림 걸린 카운터는 닫혔으면 미리 등록, **열렸으면 식이 부재를 다룬다**(`increase(x[w]) > 0 or (x unless x offset w)` — 1 로 태어난 시계열의 첫 증가를 `increase()` 가 못 읽는다, 근거: 관측(재현됨), `PrometheusRulesIT`) · 대조가 처음 돌며 찾은 것: 히스토그램 키가 미터 이름과 달라 버킷이 없었다, §9.1 이 action 값 둘과 cause 값 하나를 몰랐다 | [ADR-060](adr/ADR-060-metrics-come-from-the-table.md) |
