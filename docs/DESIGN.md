...
plan(problem, budget):
  ...
  3. 차량 할당 (Greedy, 최소 한계비용)
     ...
     - 각 클러스터에 대해 실행 가능한 차량 중 marginalCost 최소 차량 선택
       ⚠ 좌석 예약 없이는 불완전하다 — 희소 능력 차량이 일반 수요로 먼저 찬다 (4-17 에서 정정 예정)
...
**3단계의 「최소 한계비용」은 좌석 예약 없이는 불완전하다** (2026-09-12 표시,
[ADR-038](adr/ADR-038-fixed-cost-floor-is-not-a-total-cost-floor.md) · **4-17 에서 정정 예정**).
...
정정은 4-17 에서 한다 — 배정 순서(희소 수요 먼저)와 좌석 예약(용량의 일부를 남긴다) 중 어느
쪽인지는 그림자 계측이 정한다.
...
키를 재고, 바꾸게 되면 **휴리스틱 변경이므로 재기준 경로**다(§6.9).
...
**그 「차 한 대 몫」이 두 축을 빠뜨리고 있다 — 고치지 않은 채로 기록한다** (2026-09-12,
[ADR-038](adr/ADR-038-fixed-cost-floor-is-not-a-total-cost-floor.md) · [측정](benchmarks/phase4-cluster-axis.md)).
구현은 `ceil(총수요 / 가장 큰 차량의 용량)` 이고, 「수요」는 **중량과 부피뿐**이며 「차」는
**가장 큰 차**다. 계측이 둘 다 틀렸다고 말한다: 네 데이터셋의 클러스터 **64개가 전부 stop 축에
묶이고**(`bindCap = 0`), 트럭 몫으로 자른 클러스터는 밴에서 쪼개진다(`large` 899 stop ·
`peak` 1,966 stop).

고친 판(모든 하드 용량 축의 max, 그리고 **가장 작은** 차량급 기준)을 만들어 쟀더니
`small` −2.67% → −5.18% · `medium` −15.47% → −15.96% 인데 **`large` 가 −14.52% → −12.87%** 로
DoD 에서 멀어졌다. 손해는 소프트 룰 125,523원이고(`priority-boost` 의 `÷ position` 이 +83,923),
거리는 오히려 76 km 줄었다. **그리고 그 뒤에 더 큰 것이 있었다** — 고정비를 하한까지 미는
변형들은 전부 미배정 페널티를 3~4배로 문다(전부 위험물). 그래서 순서를 바꾼다: **희소 능력
좌석이 먼저다.** 만드는 법은 측정 문서에 남겼다.
...
- **최적화 항목은 구현 전에 그림자 계측으로 상한을 잰다** (2026-09-12 규칙화). 기계장치를 만들기
  ...
  같은 실행에서 함께 검사할 수 있다. Phase 4 에서 세 번 썼고 **세 번 다 결정을 바꿨다**:
...
셋 다 "지금은 못 켠다·이건 특별하다" 는 합리적인 이유로 빠졌고, 그 이유가 사라졌는지 확인할
사람이 없었다. 그래서 규칙 둘.
...
|---|---|---|---|
| 1 | **시드 행** | `DispatchAdminIT` 가 시드를 고치고 `@AfterEach` 로 되돌린다 — 병렬 실행이 들어오면 무너지는 격리 | 아직 안 깨졌다(기록만) |
| 2 | **시각** | dispatch IT 셋이 약속창을 `Instant.now()` 로 만들었다 | 21시에 돌렸더니 근무창 밖 |
| 3 | **릴레이 리더** | 발행을 보는 IT 가 리더가 되는 것이 클래스 시작 순서에 달려 있었다 | 리더 락이 Redis→advisory 로 옮겨져 **실제로 동작하기 시작**하자 |
...
| 038 | **고정비 하한은 총비용의 하한이 아니다** — 룰이 stop 상한을 스스로 말하고(`routeStopCap()`, 답하는 룰들의 **min**) · 리포트에 **완화 문제의 고정비 하한**을 상시 열로(제약 조합 × 자원, 분수 허용, 축별 max — `large` 2,500,000 대 하한 1,738,000) · **그 열은 총비용의 하한이 아니다**(고정비를 하한까지 밀면 미배정이 3~4배, 전부 위험물 — 빈 좌석은 희소 능력이 앉을 자리다) · 클러스터러 축 정정은 **만들었고 재고 넣지 않았다**(`large` −14.52% → −12.87%) — 먼저 할 것은 **희소 능력 좌석**(4-17) | 축만 고쳐서 지금 넣기(DoD 데이터셋이 1.65%p 나빠진다 — 기준은 결과를 보고 옮기는 것이 아니다), 클러스터 크기 상수 튜닝(이 항목을 만든 오류의 재발), `priority-boost` 의 `÷ position` 제거(**정책 변경** — 재는 자를 고치는 일), 하한을 정수 최적해로 올리기(하한이 아니게 된다), 총비용 하한으로 확장(완화가 원 문제만큼 어려워지면 두 번째 최적화기다), 위험물을 [ADR-031](adr/ADR-031-least-capable-first-tie-break.md) 능력 순위에 되돌리기(측정으로 뺀 것은 같은 급의 측정으로만 되돌린다 — 4-17) | [ADR-038](adr/ADR-038-fixed-cost-floor-is-not-a-total-cost-floor.md) |
