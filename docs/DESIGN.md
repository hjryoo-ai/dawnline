...
바꾸는 조건은 **미리 적었다** — 「`peak` 이 30초 안에 수렴한다」이고, 부족분은 **3초**다
(60초 예산에서 32,883 ms 에 수렴해 20,842,611). 그 3초가 어디 있는지도 쟀다: 구성 241 ms(1.7%) ·
부착 170 ms(1.2%) · **나머지 97%가 재삽입과 개선**이고, CW 가 만든 라우트 216개(차량 88대) 중
128개가 밀려 **3,880 stop 이 재삽입으로 내려가는 것**이 원인이다.
...
| 043 | **기본 전략은 `peak` 이 수렴할 때까지 바꾸지 않는다** — `savings-cw+ls` 가 네 데이터셋에서 싸지만(−29.27% · −19.24% · **−19.30%** · `overload` 111,436,423) `peak` 에서 30초 예산에 **잘리고 잘린 값이 실행마다 다르다**(20,940,782 ~ **23,277,318** — 넷째는 마감이 배정을 끊어 96 stop 이 `plan-deadline`, 기본보다 +1,767,471원) · **바꾸는 조건을 미리 적는다: `peak` 30초 수렴**(부족분 3초, 그 시간의 97%는 재삽입) · 게이트 목록에는 넣는다 · DoD 는 **−19.30%(savings, 수렴) / −14.93%(기본)** 를 함께 적는다 | 바꾸기(재현되지 않고 그 방향이 한쪽이 아니다), `peak` 예산 60초로(자를 옮겨 통과시키는 일), 크기별 기본값(「왜 이 전략인가」 축이 하나 더 생긴다 — 모드 축이 이미 답한다), 열화 사다리에 맡기기(사다리는 *직전* 계획이 조건이라 첫 계획은 언제나 전체 예산으로 돈다), 등록하지 않기(§6.6 표에 원래 있고 넷을 이긴다), 게이트에서 빼기(조용히 나빠지는 것을 볼 자리가 없어진다) | [ADR-043](adr/ADR-043-default-strategy-stays-until-peak-converges.md) |
