...
Phase 4 마감에 일곱째(검사 대상 집합)가, **Phase 5-0 에 여덟째(실행 순서)**, **Phase 5-1a 에 아홉째(환경이 결함을 가린다)**, **Phase 6 묶음 C 에 열째(빈 값끼리의 비교)와 열한째(기대값이 결함을 고정한다)**, **7-1 에 열두째(알림이 존재하지 않는 시계열을 본다)**, **7-2 에 열셋째(기본값이 기능을 조용히 끈다)와 열넷째(잘린 결과를 전체로 읽는다)** 가 붙었다.
