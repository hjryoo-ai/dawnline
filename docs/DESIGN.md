...
| `dawnline_promise_revised_total` | counter | fulfillment | camp, tier, **cause** — 하류가 상류의 약속을 개정한 횟수 (§5.2, Phase 2). `cause` 는 원래 컷오프 웨이브의 `close_cause`: `scheduled`(grace 로 흡수 못한 지연) · `manual`(운영자 조기 마감의 대가) (2026-09-24, ADR-054) |
