...
| `dawnline_routes` | gauge | **ops-api** | camp, status(assigned/in_progress/completed/unknown) — `rm_routes` 를 **진행**으로 센 집계, KPI 와 **같은 1분 갱신**에서(한 갱신이 함께 성공하고 함께 실패한다 — 알림은 `dawnline_kpi_refresh_age_seconds` 하나다). `rm_routes.status` 는 둘(`ASSIGNED` · `DEPARTED`)뿐이라 셋째는 사실에서 판정한다: `assigned` 출발 전 · `in_progress` 출발했고 결과가 없는 주문(취소 제외)이 남았다 · `completed` 출발했고 그 라우트의 주문이 전부 결과가 났다(주문 수는 stop 수가 아니라서 `ix_rmo_route` 로 남은 주문을 찾는다 — `EXISTS` 로 쓰면 플래너가 해시 서브플랜으로 바꿔 매분 `rm_orders` 전체를 훑으므로 `LATERAL … LIMIT 1` 이다. 보존 90일 1,361만 행에서 221 ms → 91 ms, 인덱스는 더하지 않는다 — [측정](benchmarks/phase7-route-progress-count.md)) · `unknown` 계획(`route.assigned`)이 아직 오지 않은 행 — 판정할 기준(그 라우트의 주문)이 없다. **부재는 값이 아니지만 부재의 수는 값이다.** 그 행의 캠프를 모르면 `camp="unknown"` 이다(`delivery.status` 는 캠프를 싣지 않는다). **범위**: 계획 출발이 정시율 창의 첫 버킷 이후인 라우트, 그리고 `unknown` 전부(탐색이 없으므로 창으로 자르지 않는다 — 모르는 것을 창으로 자르지 않는다). 창은 탐색 수를 하루치로 묶는 자리다 — 창 밖의 끝나지 않은 라우트는 여기서 빠지고 그 주문은 `dawnline_rm_orders_stuck` 이 보존 기간 끝에서 센다(재검토 지점: 하루를 넘겨 끝나지 않는 라우트가 운영 절차에 들어올 때). **라우트 단위가 아니라 집계다**: `routeId` 는 열린 라벨이고 피크일 1,250 개씩 새 값이 생겨 시계열 수가 끝없이 는다 — Prometheus 의 비용은 시계열 수이고, 한 번 생긴 시계열은 보존 기간 내내 남는다. 이 행의 시계열은 캠프 수 × 4 가 전부다. 라우트 단위 진행은 ops-web 지도(`GET /waves/{waveId}/routes`)의 몫이다. 갱신 실패 중에는 `NaN` |
