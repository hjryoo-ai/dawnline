...
**그리고 표는 기본 전략을 아직 바꾸지 않았다 — 이유는 순위가 아니라 재현성이다** (2026-09-17,
[ADR-043](adr/ADR-043-default-strategy-stays-until-peak-converges.md) ·
[측정](benchmarks/phase4-savings-cw.md) §4.3). `savings-cw+ls` 가 `small` −29.27% ·
`medium` −19.24% · `large` **−19.30%** · `overload` 111,436,423 으로 넷을 이긴다. 그런데 `peak`
에서는 30초 예산에 **잘리고, 잘린 값이 실행마다 다르다** — 같은 커밋·seed·예산으로 네 번 돌려
20,940,782 / 21,173,960 / 20,940,782 / **23,277,318** 이다. 넷째는 같은 JVM 에 전략을 넷 올린
실행이고, 마감이 개선이 아니라 **부착·재삽입을 끊어** 96 stop 이 `plan-deadline` 로 남았다 —
현재 기본 전략보다 **1,767,471원 비싸다**.
...
바꾸는 조건은 **미리 적었고, 그 조건은 구조다** — 「`peak` 에서 구성이 만든 라우트 수 ≤ 차량
수 × 1.2(= 106)」. 지금은 **216개**(차량 88대)이고 그중 128개가 부착에서 밀려 **3,880 stop 이
재삽입으로 내려간다**. 그것이 예산을 넘기는 원인이다 — 부족분 3초의 **97%가 재삽입**이고 구성은
241 ms(1.7%) · 부착은 170 ms(1.2%) 뿐이다.

**조건을 벽시계(「30초 안에 수렴」)에서 구조로 한 번 바꿨다.** 벽시계 조건은 기계가 바뀌면
참·거짓이 바뀌는데, 이 문서가 「예산이 물린 실행은 재현 대상이 아니다」라고 한 것과 같은 이유로
**전환 조건도 재현되는 것 위에 서야 한다.** 라우트 수는 같은 커밋·seed 면 같다.
...
| 베이스라인 대비 총비용 | ≥ 15% 절감 | benchmark 리포트 — `large` **−19.30% (`savings-cw+ls`, 수렴)** · 기본 전략은 −14.93% ([ADR-043](adr/ADR-043-default-strategy-stays-until-peak-converges.md)) |
