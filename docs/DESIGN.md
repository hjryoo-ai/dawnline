...
그 라우트가 부챗살 여럿을 오가는 지그재그가 된다. 클러스터의 목표 크기는 **차 한 대 몫**
(`ceil(stop 수 / 차량 수)`)이고, 경계는 그 크기의 80%를 넘긴 뒤에만 자르는 이유가 된다.
...
- **30초 예산은 계획을 묶지 않는다 — §6.5 5단계 하나를 묶는다** (2026-09-11, [측정](benchmarks/phase4-peak-gate.md) §2-2).
  `peak` 에서 개선 단계는 예산을 정확히 지켰는데(13,841 ms 를 받아 12,859 ms 사용) 계획 전체는
  **43.2초**다. 배정·재삽입에는 마감이 걸려 있지 않기 때문이다. 위 표의 목표는 「5,000 주문 /
  40 차량」이고 `large` 가 5,872 ms 로 통과하므로 목표 자체는 유효하지만, **「예산」이라는 낱말이
  계획 시간의 상한을 뜻한다고 읽으면 `peak` 에서 틀린다.** 무엇을 할지는 `docs/IMPLEMENTATION_PLAN.md`
  Phase 4 의 16번에 **[결정 필요]** 로 열려 있다.
...
둘 다 "지금은 못 켠다" 는 합리적인 이유로 꺼졌고, 켜는 날 그 이유가 사라졌는지 확인할 사람이
없었다. 그래서 규칙: **`if: false` 로 두는 job 에는 켜는 조건을 주석으로 적고, 그 조건이
사라지는 Phase 의 마감 대조표에 항목으로 넣는다.** 대조표가 이 둘을 잡은 것은 우연이 아니라
대조표가 *코드가 아니라 계획서*를 기준으로 읽기 때문이다.
