...
- **넷째 헤더 `dawnline-replay-for`** (2026-09-24, [ADR-053](adr/ADR-053-dlq-replay-is-addressed-to-the-failed-group.md)) — 앞의 셋과 달리 봉투의 중복이 아니라
  **배달의 사실**이다: 이 레코드는 DLQ 재처리이고 그 값(소비자 그룹 하나)에게만 의미가 있다. 원래 발행에는 없고
  ops-api 의 DLQ 재처리만 싣는다(§4.6). 봉투에 넣을 수 없는 이유가 곧 이 헤더의 존재 이유다 — 봉투는 value
  바이트이고, 재처리는 value 를 한 바이트도 바꾸지 않아야 `eventId` 가 유지된다. 재처리의 표시는 value 밖에만 둘 수 있다.
...
- **`processed_events` 보존: 14일** (일 1회 배치 삭제, outbox 7일 정리와 같은 정리 스케줄러 — §7.1). 근거: 재전달 가능 창의 상한은 본 토픽 보존 7일(오프셋 리셋 포함)이며 14일은 그 2배 여유다. DLQ 보존 30일은 이 창과 무관하다 — **재처리는 실패한 소비자 그룹에게만 의미 있는 사건이기 때문이다**([ADR-053](adr/ADR-053-dlq-replay-is-addressed-to-the-failed-group.md)). DLQ 레코드는 이벤트의 실패가 아니라 **그룹 하나의 실패**이고(`kafka_dlt-original-consumer-group`), 재발행은 그 그룹을 `dawnline-replay-for` 로 지목한다(§4.2). 지목된 그룹은 처리 트랜잭션이 롤백됐으므로 그 이벤트의 `processed_events` 행이 **구조적으로 없다** — 이 테이블 없이 처리된다. 지목되지 않은 그룹은 레코드를 멱등 게이트 **앞에서** 건너뛰므로 이 테이블을 읽지도 쓰지도 않는다. 두 쪽 모두 보존 기간과 무관하다.
  **(2026-09-24 정정)** 처음 문장은 「DLQ 에 들어간 이벤트는 처리 트랜잭션이 롤백된 것이므로 `processed_events` 에 성공 기록이 없고, replay 의 안전성이 이 테이블에 의존하지 않는다」였고, **실패한 그룹 하나만 보고 쓴 것이었다.** 재발행은 원래 토픽으로 나가므로 그 토픽의 모든 그룹이 받는다 — 이미 성공 처리한 다른 그룹은 `processed_events` 로만 중복을 거르고 그 행은 14일 뒤 지워지므로, 15~30일 된 레코드의 재처리는 그 그룹들에서 두 번째 처리가 됐을 것이다. 나이 상한(14일보다 오래된 레코드는 재처리 거부)은 기각했다 — 문제를 푸는 것이 아니라 DLQ 보존을 14일로 줄이는 것이고, 두 보존 기간을 묶는 조건이 하나 더 생긴다.
  **경고: 이 논거는 두 구조에 의존한다** — (1) 성공 처리된 이벤트는 DLQ 에 들어가지 않는다(§4.6), (2) 재발행은 대상 그룹을 지목하고 지목되지 않은 그룹은 **기록 없이** 건너뛴다(`libs/messaging` `ReplayTargetFilter`). 둘 중 하나를 바꾸는 변경은 이 보존 기간을 재검토해야 한다.
...
| 상황 | 처리 |
|---|---|
| 일시적 오류 (DB 연결 · 타임아웃, Redis 연결, 그리고 분류되지 않은 것) | 지수 백오프(200ms·1s·5s, 그 뒤 5초마다)로 **끝없이** 재시도한다 — 그 파티션은 풀릴 때까지 멈춘다. `dawnline_event_retry_total{reason}` 이 몇 번 · 왜를, `dawnline_event_retry_age_seconds` 가 얼마나 오래를 보인다. **(2026-09-25 정정, 7-3)** 앞의 판은 「3회」였고, 그래서 DB 장애가 DLQ 로 흘렀다(아래 「경계」) |
| 결정적 오류 (`IllegalArgumentException`, 도메인 예외) | 지수 백오프 재시도 3회 (200ms·1s·5s) 뒤 DLQ |
| 역직렬화 실패/스키마 불일치 | 즉시 DLQ + 알림 |
| 비즈니스 규칙 위반 (예: 취소 불가 상태) | DLQ 아님. 무시하고 `warn` 로그 + 메트릭 (`dawnline_event_rejected_total{reason}`) |
| DLQ 재처리 | ops-api `POST /admin/dlq/{topic}/replay` (운영자 확인 후) — 아래 「DLQ 재처리」 |

위 표는 **소비 측**이다. 발행 측에는 DLQ가 없다 — 아직 브로커에 나가지 못한 이벤트이므로 보낼 곳이 없다.

**경계 — 무엇이 일시적인가** (2026-09-25, 7-3, [ADR-015 후속 정정](adr/ADR-015-outbox-publish-side-quarantine.md)). 앞의 두 줄을 가르는 것은 ADR-015 의 **소비 측 경계표** 한 장이다(정본은 `libs/messaging` 의 `ConsumeFailure` — 표의 행 순서가 판정 순서다). 원인 사슬 전체를 보고 **표의 위에서부터 사슬 어딘가에 걸리는 첫 행**이 판정이다. 걸리는 행이 없으면 일시적이다. 원칙은 발행 측과 같다(아래 「발행 측 실패」) — **DLQ 는 독약 메시지의 자리이지 장애의 자리가 아니다.** 앞의 판(「일시적 오류 3회」)은 5분 20초 DB 장애에서 200건 중 6건을 DLQ 로 보냈고, 여섯은 재처리하자 전부 성공했다 — 독약이 아니었다(근거: 관측(재현됨) — RB-02 §3). 일시적인데 결정적으로 판정하면 이벤트가 조용히 DLQ 로 빠진다. 결정적인데 일시적으로 판정하면 파티션이 서고 알림(§9.4 `DawnlineConsumerRetryStuck`)이 울린다. 그래서 애매하면 일시적이다.
...
| | |
|---|---|
| 표면 | `GET /api/v1/admin/dlq/{topic}` 목록 → 운영자가 고른 것만 `POST /api/v1/admin/dlq/{topic}/replay` `{records:[{partition, offset}]}`(한 번에 100개까지). 이것이 위 표의 「운영자 확인 후」다. `{topic}` 은 원래 토픽이고 **계약에 있는 토픽만** 받는다(그 밖은 404, 감사 없음 — 아무것도 시도하지 않았다). 권한은 §5.5 메서드 규칙 그대로 |
| 무엇을 보내나 | **원래 바이트.** 키·value 는 그대로, 원래 토픽(`kafka_dlt-original-topic`)의 원래 파티션으로. 원래 헤더(`traceparent`·`eventType`·`schemaVersion`)는 두고 `kafka_dlt-*` 만 뺀다. `eventId` 는 value 안에 있으므로 「`eventId` 유지」는 「value 불변」과 같은 말이고, 그래서 읽기·쓰기 모두 `byte[]` 직렬화기다 — 객체로 읽었다 쓰는 경로가 없다 |
| 누구에게 | **원래 소비자 그룹에게만.** 헤더 `dawnline-replay-for=<kafka_dlt-original-consumer-group>` 을 싣는다(§4.2). 원래 그룹 헤더가 없는 DLQ 레코드는 재처리하지 않는다(`REJECTED`) — Spring 은 그 헤더를 리스너 실패일 때만 붙이고, 대상을 모르는 재처리는 모든 그룹에게 간다 |
| 다른 그룹 | 컨테이너의 레코드 필터(`libs/messaging` `ReplayTargetFilter`, 다섯 서비스 공통)가 리스너를 부르지 않고 **오프셋만 진행한다.** `processed_events` 에 **적지 않는다** — 적으면 나중에 같은 이벤트를 그 그룹 대상으로 재처리할 때 `dup` 으로 막힌다. 흔적은 `dawnline_event_processed_total{outcome="replay_not_target"}` 뿐이다(`dup` 과 섞지 않는다 — 「이미 처리했다」와 「내 일이 아니다」는 다른 사실이다) |
| 감사 | 레코드 하나에 행 하나. `action=DLQ_REPLAY`·`target_type=EVENT`·`target_id=eventId`(value 에서 못 읽으면 `NULL`)·`request={topic, partition, offset, targetGroup}`. `PENDING` 을 커밋한 **뒤에** 보낸다(§5.5 「커맨드 위임」과 같은 순서). 결과: 브로커 ack → `SUCCEEDED` · 보내기 전 거절(레코드 없음·원래 그룹 헤더 없음·원래 토픽 불일치) → `REJECTED` · 브로커의 재시도 불가 오류 → `FAILED` · 그 밖(타임아웃·재시도 가능 오류 소진·분류 불가) → `UNKNOWN`. 분류는 위 발행 측 표의 `PublishFailureClassifier` 를 그대로 쓴다 |
| 멱등 | **재처리 커맨드는 멱등이다.** 대상 그룹이 첫 번째를 처리했으면 그 그룹의 `processed_events` 가 두 번째를 `dup` 으로 막고, 다른 그룹은 두 번 다 건너뛴다. 그래서 `PENDING` 과 ack 사이에서 죽었거나 `UNKNOWN` 이면 **그대로 다시 누른다**(RB-05) |
| 목록 | 칸은 위치(파티션·오프셋)·시각·`eventType`·`eventId`·원래 그룹·예외 클래스뿐이다. **value 도 예외 메시지도 싣지 않는다**(§9.3 — 둘 다 주소를 담을 수 있다) |

**불변규칙 1 은 해당하지 않는다 — 이 발행의 상태는 감사 행이다.** 재처리는 도메인 상태를 바꾸지 않고 이미
발행된 바이트를 다시 보낼 뿐이라 outbox 를 거치지 않는다(ops-api `adapter.out.messaging`, 유스케이스는 포트만 안다).
그러나 outbox 가 없는 발행이 허용되는 이유는 **「상태가 없어서」가 아니라 「상태가 다른 곳에 있어서」다** —
`audit_logs` 의 행이 먼저 커밋되고(「보낼 것이 있다」) ack 가 그것을 닫는다. outbox 행과 `published_at` 의 순서와
같다. outbox 를 거치는 안은 기각했다: 릴레이는 봉투를 다시 조립하고, 조립하지 못하는 바이트는 격리한다 — 재처리
대상은 대개 소비자가 열지 못한 바이트다.

**재처리는 정의상 순서를 어긴다.** 재발행된 레코드는 파티션 끝에 붙으므로 같은 키의 더 새 이벤트 **뒤에** 도착한다.
§4.5 의 키 단위 순서는 재처리에 성립하지 않는다. 그것을 흡수하는 것은 소비자 다섯 자리의 축 규칙(역행 무시·판정 키 —
[ADR-017](adr/ADR-017-order-state-machine-absorbs-out-of-order-events.md) 계열과 [ADR-051](adr/ADR-051-first-fact-creates-the-row-absence-is-not-a-value.md))이지 **재처리가
순서를 지키는 것이 아니다.** ops 프로젝션에서는 `ProjectionShuffleTest`·`ProjectionShuffleIT` 의 「재처리 순서」가
그것을 본다 — 사건 하나를 빼고 나머지를 인과 순서로 흘린 뒤 그 사건을 맨 끝에 넣는 것을 **모든 사건에 대해** 돈다.
...
| `MAX_PUSHES_EXCEEDED` | **FC 선택 뒤** — 원래 컷오프의 웨이브와 밀린 셋(`MAX_WAVE_PUSHES = 3`)이 **모두 닫혀 있다**. 이벤트는 제때 왔고 받을 웨이브가 없다. 그렇게 되는 길은 사실상 운영자가 같은 캠프 · 티어의 웨이브를 컷오프 전에 거듭 닫은 것뿐이다([ADR-054](adr/ADR-054-early-wave-close-is-an-operator-cutoff.md)). 2026-09-25 까지 이 경우도 `STALE_PLACED` 로 나갔다([ADR-063](adr/ADR-063-no-open-wave-is-not-a-late-event.md)) |
...
OPEN ──(cutoff 도달, 락 획득)──▶ CLOSING ──(wave.closed 발행 완료)──▶ CLOSED ──(plan.completed)──▶ PLANNED
  └──(운영자 조기 마감, ADR-054)──┘                                      └──(plan.failed)──▶ PLAN_FAILED ──(plan.completed, 운영자 재실행)──▶ PLANNED
```
...
- 컷오프 이후 도착한 같은 티어 주문은 **다음 웨이브**로 편입. `CLOSING/CLOSED` 웨이브에는 편입 불가 — **편입이 웨이브 행을 `SELECT … FOR SHARE` 로 잡고 상태를 확인한 뒤 INSERT** 하므로, 그 트랜잭션이 끝나기 전에는 마감이 끼어들 수 없다([ADR-025](adr/ADR-025-wave-admission-share-lock.md)). 공유 락끼리는 막지 않아 같은 웨이브로 몰리는 편입은 병렬이다. **「잠근 뒤에 본 상태」는 잠근 순간의 행이다** — 같은 트랜잭션이 잠그기 전에 읽어 둔 영속성 컨텍스트의 사본이 아니다. 잠금 질의가 엔티티를 직접 돌려주던 동안 그 사본이 돌아와 닫힌 웨이브에 주문이 들어갔다(2026-09-26, [ADR-025](adr/ADR-025-wave-admission-share-lock.md) 후속 정정, 근거: 관측(재현됨) — `WaveLockedReadIT`). 잠금 질의는 `version` 만 읽고, 다르면 다시 읽는다.
...
- `dawnline_promise_revised_total{camp,tier}` — 개정 횟수. 이 값이 0 이 아니라는 것은 grace 로
  ...
  **(2026-09-24 정정, [ADR-054](adr/ADR-054-early-wave-close-is-an-operator-cutoff.md))** 앞 문장은
  `cause="scheduled"` 에만 참이다. 운영자 조기 마감이 생기면서 개정의 원인이 둘이 되었고, 라벨
  `cause` 는 **주문의 원래 컷오프 웨이브의 `close_cause`** 에서 온다 — `scheduled`(grace 로 흡수하지
  못한 지연) · `manual`(운영자가 앞당긴 컷오프의 대가, 감사 행의 `reason` 을 본다). 라벨이 없으면
  사람이 누른 결과가 「grace 가 모자라다」로 읽힌다.
...
CREATE TABLE waves (id UUID PK, camp_id UUID NOT NULL, service_tier VARCHAR(16) NOT NULL, cutoff_at TIMESTAMPTZ NOT NULL,
  ...
  close_cause VARCHAR(16),   -- SCHEDULED | MANUAL — closed_at 과 함께 채워지고 함께 비어 있다 (V3, ADR-054)
...
- **계획의 쓰기 하나는 트랜잭션 하나다**(2026-09-26 정정, [ADR-024](adr/ADR-024-plan-completed-event.md) 후속 정정 — 범위는 같은 날 [ADR-064](adr/ADR-064-planning-computes-outside-the-transaction.md) 가 쓰기로 좁혔다). 계획은 **읽기(읽기 전용 트랜잭션) → 계산(트랜잭션 없음) → 쓰기** 셋이고, 최적화기가 도는 동안 커넥션을 쥐지 않는다 — `peak` 계산 한 번이 19.9초이고 그 시각이 §8.2 의 컷오프 직전 버스트다(정정 전에는 쥐고 있었다, 근거: 관측(재현됨) — `PlanComputeConnectionIT`). `wave.closed` 의 멱등 게이트(`processed_events` 선점)는 **쓰기 트랜잭션**을 감싸고, 그 안에서 계획 열기(`REQUESTED`) · `PLANNING` 전이 · 결과 쓰기 · 세 이벤트가 함께 커밋되고, 커밋은 `PUBLISHED` 나 `FAILED` 로만 일어난다. 그래서 **계획 중 인스턴스가 죽으면 아무것도 남지 않는다** — 계산 중이면 쓴 것이 아직 없고, 쓰기 중이면 트랜잭션이 롤백되고 커밋되지 않은 오프셋이 `wave.closed` 를 다시 전달해 재기동 즉시 다시 계획한다. 부분 결과가 발행되지 않는다. `PLANNING` 은 트랜잭션 안의 상태라 다른 세션에는 보이지 않는다 — 계획 중인 웨이브는 `route_plans` 에 행이 없다. 이전 판은 「`PLANNING` 으로 남고 10분 뒤 스케줄러가 회수한다」고 적었고 그 스케줄러가 있었지만, 커밋된 `PLANNING` 이 생기는 경로가 없었다(근거: 관측(재현됨) — 결과 쓰기에서 세운 계획을 `SIGKILL` 로 죽여도 행 0, 재기동 26초 뒤 `PUBLISHED`). 지웠다.
...
**후보가 없는 라우트는 고치지 않고 실패한다**(2026-09-25, [ADR-059](adr/ADR-059-dispatch-retention-is-per-plan.md) 결정 3).
위 대안은 버렸지만 **화물과 시각 재계산은 여전히 후보를 읽는다** — 재배정·부분 재계획(§6.8)·취소 반영(§6.10)이
라우트를 다시 쓸 때 무게·부피·약속창을 `dispatch_candidates` 에서 가져온다(`JdbcRouteMutations`). 그 조인이 내부
조인이던 동안 후보 없는 주문은 **조용히 빠졌고**, 빠진 목록이 비면 재배정은 출발 라우트를 비웠다(200). 이제 그 자리
넷(`loadPositionedStops` · `snapshot` · `moveOrder` · `cancelStopIfAllOrdersCancelled`)은 후보가 하나라도 없으면
...
- **캠프 코드의 변경은 반영되지 않는다 — 참조 데이터는 버전이 없다.** fulfillment 의 캠프는 참조 데이터이고,
  ...
  - 코드가 바뀐 뒤에 마감된 웨이브는 새 코드를 싣는다. 그러면 한 캠프의 행에 옛 코드와 새 코드가 섞이고,
    `GET /camps` 는 그중 `max(camp_code)`(사전순)를 보인다 — **최신 코드라는 보장이 없다**.
  - 알고 두는 한계다. 화면의 캠프 코드는 표시용이고 식별은 `campId` 가 한다. 재검토 지점: 캠프 참조 데이터가
    변경 이벤트를 갖게 될 때, 또는 코드 변경이 운영 절차에 들어올 때.
...
CREATE TABLE rm_routes (route_id UUID PK, plan_id UUID, camp_id UUID, vehicle_id UUID, driver_id UUID,
  ...
  updated_at TIMESTAMPTZ NOT NULL,                    -- 보존의 나이 (V6, 2026-09-25, ADR-058)
  live_count INTEGER,                                 -- 집계 — 비취소 주문 수, 0 이면 void (V8, 2026-09-25, ADR-061)
  completed_at TIMESTAMPTZ);                          -- 집계 — 남은 주문이 없을 때의 마지막 결과 시각, void 는 NULL (V8, ADR-061)
...
- **`rm_routes.completed_at` · `live_count`** (2026-09-25, [ADR-061](adr/ADR-061-unfinished-work-has-no-window.md)). 「완료」는
  `status` 의 칸이 아니라 판정이다(`ASSIGNED` · `DEPARTED` 둘뿐) — 그 판정을 매분 읽기에서 하지 않고 **판정의 입력이
  바뀔 때 그 라우트만** 다시 센다. `completed_count` · `failed_count` 와 같은 재집계 질의가 쓰는 **집계 칸**이다(핸들러가
  쓰지 않는다, ADR-051 결정 4). `live_count` 는 비취소 주문 수이고 **0 이면 그 라우트는 void** 다 — 할 일이 없다(재계획이
  주문을 전부 옮겼거나 전부 취소됐다). `completed_at` 은 비취소 주문이 있고 그중 결과 없는 것이 남지 않았으면 주문들의
  마지막 결과 시각, 아니면 `NULL` 이다. **void 에는 시각을 만들지 않는다** — 계획 출발을 완료 시각으로 적으면 일어나지 않은
  완료에 시각이 생긴다(부재를 값으로 읽지 않는 것과 값을 만들어 내지 않는 것은 같은 규칙의 양면이다). 전부 사실이라 처리
  순서를 타지 않는다.
  다시 세는 사실은 셋이다 — `route.assigned`(주문 집합) · `delivery.status`(결과) · `order.cancelled`(남은
  주문에서 빠진다). 마지막 것은 라우트의 칸을 쓰지 않고 **다시 세기만** 한다 — 위 「`rm_routes` 에 넷」은 칸을 쓰는
  토픽의 수다. 주문의 라우트를 알아야 하므로 주문을 먼저 잠근다(잠금 순서 그대로).
...
(`KpiGauges` — 2026-09-25 까지 `OnTimeRatioGauges`). **창은 「직전 24시간」이 아니라 「현재 버킷 포함 UTC 정시 버킷 24개」다** — 현재
...
- **지도는 기본이 타일 없음이다**(ADR-057 결정 1, `[결정 필요]` 5 해소): Leaflet 위에 타일 레이어 없이 창고 · stop · 순서 폴리라인만 — 캠프 중심, 도로 없음. OpenStreetMap 타일은 설정에서 켜는 **데모용 선택**이고(적은 빈도, 표시와 함께), 타일이 안 와도 화면은 깨지지 않는다 — 컴포넌트 테스트가 그것을 본다. 데모는 인터넷 없이 선다.
- **컨테이너는 nginx 이미지 하나다**(ADR-057 결정 2) — Buildpacks 가 아닌 유일한 이미지이고, `make images`(그래서 CI 의 Compose 스모크)가 함께 만든다. nginx 가 `/api/` 를 ops-api 로 넘겨 같은 출처다 — CORS 를 열지 않는다. `Authorization` 과 응답의 `X-Dawnline-Audit-Id` 의 통과는 스모크가 본다(묶음 C3).
...
- `sim-runner` (Spring Boot CLI, `tools/sim-runner`): 시나리오 YAML로 (a) 주문 생성기 — 캠프별 좌표 분포, 티어 비율, 냉장 비율, 초당 rps 곡선(평시·피크) (b) 기사 시뮬레이터 — `route.assigned` 구독 후 stop을 순서대로 이동하며 스캔 이벤트 호출, 지연·실패 확률 주입.
  ...
  **시뮬레이션 시계 — 스케줄이 아니라 시계를 옮긴다** (2026-09-26, 7-4a, [ADR-066](adr/ADR-066-simulation-moves-the-clock-not-the-schedule.md)).
  컷오프는 `TierSchedule` 의 상수(DAWN 00:00 · SAME_DAY 10:00 · 14:00 KST)이고 설정할 자리가 없다 — 「컷오프 직전 버스트」
  (`peak-day`)는 벽시계로는 하루에 한 번 온다. 그 문장은 시계가 벽시계일 때만 참이다: 다섯 서비스와 sim-runner 는 `libs/messaging`
  의 `dawnlineClock` 하나에서 시각을 받고, 그 시계에 `dawnline.clock.offset` 을 더하면 스케줄 · 도메인 · 파이프라인을 하나도
  바꾸지 않고 「실행 시작이 22:40 KST」를 아무 때나 만든다. 조건 넷:
  - **한 값, 한 자리** — compose `x-service-env` 앵커의 `DAWNLINE_CLOCK_OFFSET`. 기동 로그에 「시계: 유효 시각 …」 한 줄, 게이지
    `dawnline_clock_offset_seconds`(§9.1), `make obs-check` 6 이 서비스 다섯의 값이 하나인지 대조한다.
  - **시뮬레이션 프로필에서만** — 0 이 아닌 오프셋은 프로필 `sim` 이 없으면 기동을 거부한다. 운영 프로필이 따로 없으니 「운영이면 거부」가
    아니라 「시뮬레이션이라고 말한 곳에서만 허용」이다.
  - **시계는 하나** — 운영 코드의 SQL 은 DB 시계(`now()` · `CURRENT_TIMESTAMP` …)를 읽지 않는다. 「현재」는 파라미터다. 남아 있던 셋
    (outbox 미발행 나이 — 오프셋 아래에서 **−28,799.95초**, 관측 · 재배정의 새 stop 자리표시 · 룰 `updated_at`)을 걷어냈고
    `SqlUsesInjectedClockTest` 가 재발을 막는다(마이그레이션은 제외 — 이유는 테스트에).
  - **시간은 뒤로 가지 않는다** — 오프셋을 켜고 쓴 사실은 벽시계보다 미래라, 같은 볼륨으로 오프셋 0 에 돌아오면 닫힌 웨이브가 아직 오지 않은
    컷오프를 들고 있다. 시뮬레이션은 자기 compose 프로젝트(`dawnline-sim`, 볼륨 `dawnline-sim_*`)에서 돌고, 연달아 돌 때는 이미 적힌 가장 늦은
    사실보다 뒤를 고른다. `make sim-up [SIM_AT=22:40]`(`tools/sim/sim-stack.sh`)이 그 셋 — 프로젝트 · 오프셋 계산 · 앵커 — 을 한다.

  **벽시계로 남는 것** — 사실은 주입 시계, 인프라는 벽시계다: JWT `iat`·`exp`(발급 스크립트 · 검증기 모두), Redis TTL(쿨다운 · 락 임대),
...
| `dispatch_candidates` | 30일 | 그 계획의 `finished_at` | 계획이 **종결**(발행·실패 ∧ 그 계획의 모든 stop 종결)인 웨이브의 후보 전부, 계획 단위. 끝나지 않은 계획의 후보는 남고 `dawnline_route_plans_stuck` 이 센다 | `dawnline.dispatch.retention.candidates` | [ADR-059](adr/ADR-059-dispatch-retention-is-per-plan.md) |
| `dispatch_candidates` — 상한 | 365일 | `updated_at` | 계획이 없는 웨이브의 후보 — **정리이지 정책이 아니다.** 계획이 있는 웨이브의 후보는 그 계획과 함께만 지운다 | `dawnline.dispatch.retention.cap` | [ADR-059](adr/ADR-059-dispatch-retention-is-per-plan.md) |
| `plan_explanations` | 30일 | 그 계획의 `finished_at` | 종결 계획의 설명 전부, 계획 단위 | `dawnline.dispatch.retention.explanations` | [ADR-059](adr/ADR-059-dispatch-retention-is-per-plan.md) |
| `route_plans` | 90일 | `finished_at` | 종결 계획 — **계열째 한 트랜잭션에서 자식부터** 지운다(아래 셋과 남은 설명·후보) | `dawnline.dispatch.retention.plans` | [ADR-059](adr/ADR-059-dispatch-retention-is-per-plan.md) |
| `routes` | 90일 | 그 계획의 `finished_at` | 계획과 함께 | `dawnline.dispatch.retention.plans` | [ADR-059](adr/ADR-059-dispatch-retention-is-per-plan.md) |
| `route_stops` | 90일 | 그 계획의 `finished_at` | 계획과 함께 | `dawnline.dispatch.retention.plans` | [ADR-059](adr/ADR-059-dispatch-retention-is-per-plan.md) |
| `route_stop_orders` | 90일 | 그 계획의 `finished_at` | 계획과 함께 | `dawnline.dispatch.retention.plans` | [ADR-059](adr/ADR-059-dispatch-retention-is-per-plan.md) |
| `route_plans` — 상한 | 365일 | `COALESCE(finished_at, started_at)` | 전부, 계열째 — **정리이지 정책이 아니다.** 그 전까지(30일부터) `dawnline_route_plans_stuck` 이 센다 | `dawnline.dispatch.retention.cap` | [ADR-059](adr/ADR-059-dispatch-retention-is-per-plan.md) |
...
**dispatch 의 여섯 표는 계획 단위로 지운다**([ADR-059](adr/ADR-059-dispatch-retention-is-per-plan.md), 7-0c — 기간은 ADR-058 결정 9 가 정했다). 위 「부모는 참조하는 자식이 없을 때만」과 모양이 다르다: 계획 계열은 FK 가 넷이라 표마다 배치를 돌리면 부모 쪽마다 가드가 필요하므로, **계획 하나를 한 트랜잭션에서 자식부터**(`route_stop_orders` → `route_stops` → `routes` → `plan_explanations` → `dispatch_candidates` → `route_plans`) 지운다. 그래서 계열 네 표는 설정 키가 하나이고, 성공 나이 게이지의 여섯 라벨이 함께 움직인다. 모든 문장이 기존 인덱스의 앞머리를 타고 새 인덱스는 없다 — 트랜잭션의 상한은 「계획 하나」, 벤치마크 `peak` 계획이면 평상시 약 2.4만 · 최악 약 5.4만 행이다([측정](benchmarks/phase7-dispatch-retention.md)). **나이는 계획의 `finished_at` 이고 이것은 안전한 쪽이 아니다** — 계획 뒤의 사실(stop 전이·재계획·재배정)은 배송일 안에 끝난다는 전제 위에서 최대 하루 일찍 지운다. **후보는 계획이 종결일 때만 지운다** — 재배정·재계획·취소가 후보를 조인하기 때문이고(§5.3), 그 조인은 후보가 없으면 stop 을 빼지 않고 `candidates-expired`(409)로 실패한다.
...
**짝 — 여유는 양 끝에 있다** (2026-09-25): 파티션 회전은 앞을 만들고 뒤를 지우는데 `dawnline_shipment_partitions_ahead`
는 앞만 잰다 — **지우기가 멈추면 앞은 멀쩡한 채 뒤가 자란다.** 뒤의 여유는
`dawnline_retention_last_success_age_seconds{table="shipment_events"}` 가 잰다([ADR-058](adr/ADR-058-shipment-and-read-model-retention.md) 결정 6).

**짝 — 없는 시계열은 0 으로 보인다** (2026-09-24). 처음 셀 때 만들어지는 카운터는 사건이 한 번도 없으면
시계열 자체가 없고, `> 0` 알림은 「0」과 「없음」을 구별하지 못한다. 첫 사건에서 시계열이 **1 로 태어나면**
`increase()` 는 앞 표본이 없어 그 첫 증가를 0 으로 읽는다 — 가장 중요한 첫 번째를 놓친다(**근거: 관측(재현됨)**,
2026-09-25 — `PrometheusRulesIT` 가 실제 Prometheus 컨테이너로 테스트의 Micrometer 레지스트리를 긁는다. 미리 등록한
카운터와 처음 셀 때 만든 카운터를 같은 순간 올리면 `increase()` 는 앞의 것만 읽고 뒤의 것은 0 이다. 같은 실행에서 규칙
파일의 부재를 다루는 식은 뒤의 것에 울린다. 처음에는 「추정」이었다 — Prometheus 계측 지침 「Avoid missing metrics」가
...
없고, 규칙 파일에서 부재를 다루는 식으로 푼다. **「남은 하나」는 둘이었다** (2026-09-25, [ADR-060](adr/ADR-060-metrics-come-from-the-table.md)):
§9.4 「DLQ 신규」의 `dawnline_event_processed_total{outcome="dlq"}` 도 `consumer`·`eventType` 이 열려 있다. 이 문단이 하나만 안
이유는 열림과 닫힘이 **산문에만** 있었기 때문이다. 이제 라벨 칸의 문법(위)이 그것을 적고 카탈로그가 든다. 규칙 파일의 식이
열린 카운터를 참조하면 `AlertRulesConsistencyTest` 가 아래 모양을 요구한다 — 뒤의 절이 「창 w 전에는 없던 시계열」, 곧 1 로
태어난 첫 증가다:

```
increase(x[w]) > 0  or  (x unless x offset w)
```
...
**이 표는 코드의 입력이다** (2026-09-25, [ADR-060](adr/ADR-060-metrics-come-from-the-table.md)). `libs/observability` 의
카탈로그 `DawnlineMetrics` 가 행마다 항목 하나를 들고, `DawnlineMetricsTest` 가 이 표를 읽어 **양방향으로** 대조한다 — 이름 ·
타입 · 라벨 집합. 서비스는 그 항목으로만 미터를 등록하고(`DawnlineMeters`, ArchUnit 규칙 11), 대시보드와 알림 규칙(§9.4)이 쓰는
`dawnline_*` 이름도 이 표에 있어야 한다. 행을 더하거나 라벨 값을 바꾸면 그 모든 곳이 같은 PR 에서 맞아야 초록이다.

**라벨 칸의 문법** — 대조 검사가 이렇게 읽는다. 칸의 앞부분, 괄호 밖의 첫 ` — ` 까지가 라벨 목록이다. 라벨은 쉼표로 가른다.
- `key(v1/v2)` 는 **닫힌** 라벨이다. 코드가 값을 전부 알고, 괄호 안의 값이 그 전부다. 값은 `/` 나 `·` 로 가르고, 괄호 안의
  ` — ` 뒤는 설명이다.
- 괄호가 없는 `key` 는 **열린** 라벨이다. 캠프 · 소비자 · 이벤트 타입처럼 코드가 값의 목록을 모른다.
- `라벨 없음` 은 빈 집합이다.
- 굵게와 코드 표기는 무시한다.

닫힘과 열림은 알림에서 갈린다(아래 「짝」). 닫힌 카운터에 걸린 알림은 기동 때 조합 전부를 0 으로 등록하고, 열린 카운터에
걸린 알림은 식이 부재를 다룬다.

**`histogram` 은 버킷이다.** 타입 칸이 `histogram` 인 항목은 헬퍼가 `publishPercentileHistogram()` 으로 등록한다. 속성 파일의
`percentiles-histogram` 키로 켜던 때는 키가 Prometheus 이름이라 **미터 이름과 맞지 않아 버킷이 없었다**(근거: 관측(재현됨),
ADR-060 맥락 1) — §9.4 의 p95 알림이 읽을 것이 없었다.

| 메트릭 | 타입 | emit 주체 | 라벨 |
|---|---|---|---|
| `dawnline_orders_placed_total` | counter | order | tier(DAWN/SAME_DAY/NEXT_DAY) — **camp 는 없다**. 캠프는 fulfillment 가 정하므로(§5.2) 접수 시점에는 존재하지 않는다. 캠프별 유입은 `dawnline_wave_orders` 가 본다 |
| `dawnline_idempotent_replays_total` | counter | order | tier(DAWN/SAME_DAY/NEXT_DAY) — 같은 멱등 키의 재요청으로 저장된 응답을 재생한 횟수. `orders_placed` 와 함께 보면 **클라이언트 재시도 폭주와 실제 주문 증가를 구분**할 수 있다 |
...
| `dawnline_event_processed_total` | counter | 전 소비자 | consumer, eventType, outcome(ok/dup/rejected/dlq/**replay_not_target**) — `replay_not_target` 은 다른 그룹을 지목한 DLQ 재처리를 멱등 게이트 **앞에서** 건너뛴 것이다(§4.6, [ADR-053](adr/ADR-053-dlq-replay-is-addressed-to-the-failed-group.md)). `dup` 과 합치지 않는다 — `dup` 은 「이미 처리했다」이고 이것은 「내 일이 아니다」라, 합치면 `dup` 이 는 것이 재전달인지 재처리인지 가릴 수 없다. 이 결과만 `processed_events` 에 흔적이 없다 |
...
| `dawnline_event_retry_total` | counter | 전 소비자 | consumer, reason(redis/db_connection/db_resource/db_transient/db_integrity/argument/domain/other) — **재시도 경로에 든 실패한 배달.** `reason` 은 [ADR-015 후속 정정](adr/ADR-015-outbox-publish-side-quarantine.md) 경계표의 행 이름이다(값은 enum `ConsumeFailure` 에서 나오고 카탈로그와 대조된다). 결정적 행(`argument` · `domain`)은 레코드당 최대 4 — 첫 배달과 재시도 셋이고 넷째 실패가 DLQ 로 간다. 일시적 행은 풀릴 때까지 오른다. `deserialization` 은 값이 아니다 — 재시도 목록 밖이라 즉시 DLQ 로 가고 `dawnline_event_processed_total{outcome="dlq"}` 만 센다. 이 카운터는 「몇 번 · 왜」를 센다. 「얼마나 오래」는 다음 행이 잰다 |
| `dawnline_event_retry_age_seconds` | gauge | 전 소비자 | consumer — **지금 재시도 중인 레코드가 파티션을 막고 있는 시간(초), 이 인스턴스의 파티션 중 최대.** 재시도 중이 아니면 0 이다. 레코드의 첫 실패에서 시작하고, 그 레코드가 성공하거나 · DLQ 로 가거나 · 파티션이 이 인스턴스를 떠나면 지운다. 파티션 정지의 신호는 「몇 번」이 아니라 「얼마나 오래」다 — 위 「남은 여유」 원칙의 모양이고, 멈춘 파티션은 시계만 지나도 값이 오른다. 알림은 §9.4 `DawnlineConsumerRetryStuck`(30분) — **일시적 실패가 30분 넘게 이어지면 그건 장애가 아니라 설정이다** |
| `dawnline_wave_orders` | gauge | fulfillment | camp, tier(DAWN/SAME_DAY/NEXT_DAY) — 마감 시점의 편입 주문 수. `waves.order_count` 는 마감 전 0 이므로([ADR-025](adr/ADR-025-wave-admission-share-lock.md)) 이 값이 편입량의 유일한 관측 경로다. 스크레이프마다 집계하지 않고 **마감할 때 이미 센 값**을 남긴다 — 관측이 §8.2 피크에 부하가 되면 안 된다 |
...
| `dawnline_promise_revised_total` | counter | fulfillment | camp, tier(DAWN/SAME_DAY/NEXT_DAY), **cause**(scheduled/manual/unknown) — 하류가 상류의 약속을 개정한 횟수 (§5.2, Phase 2). `cause` 는 원래 컷오프 웨이브의 `close_cause`: `scheduled`(grace 로 흡수 못한 지연) · `manual`(운영자 조기 마감의 대가) (2026-09-24, ADR-054). **`unknown`** 은 닫힌 웨이브가 `close_cause` 를 갖지 않은 경우다 — 커밋된 행에서는 나올 수 없고(V3 CHECK), 보이면 그 불변식이 깨졌다. 개정은 세야 하므로 예외 대신 이 값으로 센다. 표가 이 값을 적지 않고 있던 것을 카탈로그 대조가 찾았다(2026-09-25, ADR-060) |
...
| `dawnline_geo_lookups_total` | counter | fulfillment | index(fc/zone/wave_lock), outcome(redis/bypassed) — `bypassed` 는 Redis 를 건너뛰고 DB 로 답한 것이다(§7.2): `fc` 는 DB 전체 조회 + 메모리 하버사인, `zone` 은 권역 캐시를 건너뛴 DB 조회, `wave_lock` 은 락 없이 진행한 웨이브 마감. **이름과 달리 GEO 만이 아니다** — fulfillment 의 Redis 폴백 셋이 이 카운터를 함께 쓴다. 표가 `index` 를 GEO 의 둘로만 알고 있던 것을 등록 헬퍼의 값 검사가 찾았다(2026-09-25, ADR-060). 레이트 리밋의 `bypassed` 와 같은 어휘를 쓴다 — **폴백은 조용히 일어나면 안 된다** |
| `dawnline_plan_duration_seconds` | histogram | dispatch | strategy, mode(FULL/FAST), **termination**(converged/deadline) — **알고리즘 시간만**이다(§6.7). 영속화는 아래 짝이 잰다. `termination` 은 2026-09-24 에 붙었다: `deadline` 이면 마감에 잘려 하지 못한 일이 있고 그 결과는 그날의 기계 속도에 달린다([ADR-036](adr/ADR-036-deadline-belongs-to-the-plan.md)) — 운영에서는 **잘린 계획의 비율**이고, CI 에서는 시간 대신 보는 값이다 |
...
| `dawnline_routes` | gauge | **ops-api** | camp, status(assigned/in_progress/completed/void/unknown) — `rm_routes` 를 **진행**으로 센 집계, KPI 와 **같은 1분 갱신**에서(한 갱신이 함께 성공하고 함께 실패한다 — 알림은 `dawnline_kpi_refresh_age_seconds` 하나다). `rm_routes.status` 는 둘(`ASSIGNED` · `DEPARTED`)뿐이라 완료는 판정이다 — 그 판정은 **쓰기 때** 한다(`rm_routes.completed_at` · `live_count`, §5.5 · [ADR-061](adr/ADR-061-unfinished-work-has-no-window.md)): `assigned` 출발 전 · `in_progress` `status = 'DEPARTED' AND completed_at IS NULL` 이고 void 가 아니다 — 출발했고 결과 없는 주문(취소 제외)이 남았다 · `completed` 출발했고 남은 주문이 없다(주문 수는 stop 수가 아니라서 `completed_count + failed_count` 를 `stop_count` 와 견주지 않는다) · `void` 비취소 주문이 없다(`live_count = 0`, 출발 여부와 무관) — 할 일이 없는 라우트는 완료가 아니다: 한 번도 돌지 않은 라우트를 `completed` 에 섞지 않고, 일어나지 않은 완료에 시각을 만들지 않는다 · `unknown` 계획(`route.assigned`)이 아직 오지 않은 행 — 판정할 기준(그 라우트의 주문)이 없다. **부재는 값이 아니지만 부재의 수는 값이다.** 그 행의 캠프를 모르면 `camp="unknown"` 이다(`delivery.status` 는 캠프를 싣지 않는다). **범위 — 끝나지 않은 일에는 창이 없다**: `assigned` · `in_progress` · `unknown` 은 창 없이 전부, `completed` · `void` 는 계획 출발이 정시율 창의 첫 버킷 이후인 것만(수를 묶는 창). 출발한 지 30시간 된 라우트가 아직 끝나지 않았다면 운영자가 가장 먼저 볼 라우트다 — 떠나지 않은 라우트도 같다 — `dawnline_fulfillment_orders_stuck` 에 상한이 없는 것과 같은 이유다. 질의는 `rm_routes` 하나만 읽는다(보존 90일 11만 행에서 순차 스캔 7–11 ms, 인덱스 없음 — [측정](benchmarks/phase7-route-progress-count.md)). 처음(7-1, #68)에는 읽기가 라우트마다 `rm_orders` 를 찾았고 그 탐색을 묶으려고 전부에 창을 걸었다 — 창 밖의 진행 중 라우트가 빠졌다(운영 크기에서 471 중 55). **라우트 단위가 아니라 집계다**: `routeId` 는 열린 라벨이고 피크일 1,250 개씩 새 값이 생겨 시계열 수가 끝없이 는다 — Prometheus 의 비용은 시계열 수이고, 한 번 생긴 시계열은 보존 기간 내내 남는다. 이 행의 시계열은 캠프 수 × 5 가 전부다. 라우트 단위 진행은 ops-web 지도(`GET /waves/{waveId}/routes`)의 몫이다. 갱신 실패 중에는 `NaN` |
...
| `dawnline_clock_offset_seconds` | gauge | 전 서비스(`libs/messaging`) | service — 주입 시계가 벽시계보다 앞선 초([ADR-066](adr/ADR-066-simulation-moves-the-clock-not-the-schedule.md)). 0 이면 벽시계, 0 이 아니면 **시뮬레이션 시각**이다 — 그래프의 x 축은 실제 시각이고 사실의 시각은 거기에 이 값을 더한 것이다. 기동 때 한 번 등록하고 바뀌지 않는다. `make obs-check` 6 이 서비스 다섯의 값이 **하나**인지 대조한다(같은 앵커에서 읽는다는 문장은 적어 둔 문장일 뿐이다). 레지스트리가 없는 도구(sim-runner)는 내지 않는다 |
...
| `dawnline_fulfillment_orders_stuck` | gauge | fulfillment | 라벨 없음 — 보존 기간(30일)을 넘겼는데 웨이브가 아직 **계획되지 않은** `fulfillment_orders` 행 수([ADR-058](adr/ADR-058-shipment-and-read-model-retention.md) 결정 8). 이 행은 정리가 나이와 무관하게 남기는 행 단위 예외이고 나이가 아니라 해소(웨이브 계획)로 닫힌다 — **예외는 셈이 있어야 예외다.** `dawnline_rm_orders_stuck` 과 같은 모양이다: 정리 배치가 돌 때 세고, 세기 전과 세기에 실패한 동안은 `NaN`. 상한은 없다 — 조사 대상이 아니라 끝나지 않은 일이고, 0 이 아니면 볼 곳은 RB-04 다. 다른 행 단위 예외(격리된 outbox 행)는 `dawnline_outbox_failed` 가 센다 |
...
Kafka 프로듀서 지표와 **클라이언트** 랙은 Spring Kafka 기본 지표를 쓴다 — 다만 **클라이언트 랙은 가져오지 못한 양이지 처리하지 못한 양이 아니다**(「브로커의 끝 − 가져온 위치」 — 가져왔지만 재시도에 막힌 레코드를 세지 않는다. 근거: 관측(재현됨) — 2026-09-25 `make chaos-db`, 그룹 랙 합 약 1,200 에 클라이언트 `records_lag` 합 최대 500 · `records_lag_max` 최대 102, RB-02 §3). 그래서 대시보드용으로 내린다. **알림이 보는 랙은 브로커의 사실**이다 — kafka-exporter 의 `kafka_consumergroup_lag{consumergroup, topic, partition}`(커밋된 오프셋 기준, §11). 표에 없는 이름이고 있다는 것은 `make obs-check` 가 본다.
...
**정정 (2026-09-25, 7-2, [ADR-062](adr/ADR-062-trace-survives-the-outbox.md))** — 계획은 웨이브 마감이 시작하고 주문 여럿을 모으므로
주문 하나의 트레이스는 tracking 에 닿지 않는다. 한 주문의 일은 **두 트레이스**이고 **`dawnline.wave_id` 속성 하나로 함께 찾는다**:
...
한도를 1000 으로 올리고 결과가 한도에 닿으면 실패로 둔다(§13 축 14).
...
1. **쓰기**: outbox 행의 `headers.traceparent` 는 쓰는 트랜잭션의 현재 스팬이다(`TracerTraceparentSupplier`). 7-2 이전에는
   제공자가 기본값 `NONE` 하나뿐이라 **한 번도 실리지 않았다**(§13 축 13).
2. **발행**: 릴레이는 저장된 `traceparent` 를 부모로 하는 수신 관측(`outbox relay`) 안에서 보낸다. 템플릿 관측이 헤더를
   지우고 현재 컨텍스트로 다시 쓰므로(spring-kafka `KafkaRecordSenderContext`), 그 관측이 없으면 릴레이 폴링의 트레이스로
   덮인다. 발행 스팬은 그 자식이라 릴레이 지연이 트레이스에 보인다. 관측이 만드는 미터 `outbox_relay_seconds` 는 `dawnline_*`
   밖이라 §9.1 카탈로그의 대상이 아니다.
3. **소비**: Boot 가 만든 리스너 팩토리의 관측(`spring.kafka.listener.observation-enabled`)이 헤더를 부모로 소비 스팬을 연다.
   리스너 안에서 쓴 outbox 행이 다시 1 로 간다.

검사: 1·2 는 `OutboxTraceparentIT`, 3 은 `ListenerTraceparentIT`(둘 다 `libs/messaging`), 전 구간은 Compose 스모크.
**`management.tracing.export.enabled=false` 는 전파까지 끈다** — Boot 4.1 의 W3C 전파기 빈이 `@ConditionalOnEnabledTracingExport`
다. 내보내기만 끄려면 `management.tracing.export.otlp.enabled=false`(lean 구성, 테스트)를 쓴다.

### 9.3 로깅

JSON 구조 로그(traceId, spanId, service, eventId, orderId/waveId/routeId MDC). 개인정보(주소 전체)는 로그에 남기지 않는다(우편번호·geohash만).
**id 는 스팬 속성으로도** (2026-09-25, 7-2, [ADR-062](adr/ADR-062-trace-survives-the-outbox.md) 결정 4): `MdcScope` 가 MDC 에 넣는
`orderId` · `waveId` · `routeId` 를 현재 스팬에도 단다. 이름은 MDC 키에서 규칙으로 나온다 — `dawnline.` + snake_case
(`waveId` → `dawnline.wave_id`). 두 이름을 따로 적지 않는 이유: 로그와 트레이스가 같은 id 를 다른 이름으로 부르기 시작하면
「이 로그의 웨이브를 트레이스에서 찾는다」가 번역이 된다. `MdcScope` 는 트레이싱이 없는 도구도 쓰므로 OpenTelemetry API 가
있을 때만 단다.
...
- `Order Intake`: rps, p99, 429/5xx, outbox 지연, 레이트 리밋 판정
- `Waves & Plans`: 웨이브별 주문 수, 계획 시간, 비용, 미배정, degraded, **재계획**(Phase 5 카운터 넷 — 아래)
- `Delivery`: 정시율, at-risk, 실패(`dawnline_kpi_delivery`), 라우트 진행(`dawnline_routes` — 캠프 단위 집계, 라우트 단위는 ops-web 지도)
- `Platform`: consumer lag, DLQ 건수, DB 커넥션, JVM, outbox 리더 · 격리, 보존(나이 · 걸린 행), 시계 오프셋(ADR-066 — x 축은 실제 시각, 사실의 시각은 거기에 이 값을 더한 것)

**커밋된 JSON 은 프로비저닝 전용이다** — UI 에서 저장하지 않는다(`deploy/compose/README.md`). Grafana 는 저장할 때 키를 재정렬하고
id 를 붙여 diff 를 뜻 없이 키운다. 그래서 대조는 파일이 아니라 **메트릭 이름 집합**으로 한다(`DashboardsConsistencyTest`, ADR-060).
- 패널과 규칙이 쓰는 `dawnline_*` 이름은 전부 §9.1 에 있어야 한다.
- §9.1 의 모든 행은 패널이나 규칙 어딘가에 나와야 한다. 빼는 행은 테스트에 이유와 함께 적는다.
...
**알림 규칙** (`deploy/compose/prometheus/rules/dawnline-alerts.yml`). 이 표가 그 파일의 목록이다 — `AlertRulesConsistencyTest` 가
알림 이름 집합을 양방향으로 대조하고, 식이 참조하는 **열린 카운터**(§9.1 라벨 칸)가 부재를 다루는 모양인지 본다. 식의 동작은
`deploy/compose/prometheus/tests/` 의 promtool 단위 테스트가 본다(`PrometheusRulesIT` 가 컨테이너에서 돌린다).
표 밖의 값 — 창(w)과 `for` — 는 규칙 파일이 정본이고, 여기에는 뜻만 적는다.

| 알림 | 조건 | 뜻 · 볼 곳 |
|---|---|---|
| `DawnlineOutboxLag` | `dawnline_outbox_lag_seconds` > 30 | 가장 오래된 미발행 outbox 행이 30초를 넘었다 — 릴레이가 멈췄거나 브로커가 받지 않는다. 왜인지는 `dawnline_outbox_leader`(0 · -1)가 말한다. RB-01 |
...
| `DawnlineConsumerLag` | `sum by (consumergroup, topic) (kafka_consumergroup_lag >= 0)` > 1,000 | **밀림** — 브로커가 아는 랙(커밋된 오프셋, kafka-exporter)의 그룹 · 토픽별 합이 1,000 을 넘었다. RB-01. **(2026-09-26 정정, 7-3)** 처음 식은 클라이언트의 `records_lag_max`(컨슈머 하나의 파티션 중 최대)였다 — 1,200 건이 밀린 DB 장애에서 102 였고, 클라이언트 합으로 바꿔도 500 이었다(가져왔지만 재시도에 막힌 레코드를 세지 않는다 — §9.1, 근거: 관측(재현됨)). **정지는 이 알림의 일이 아니다** — 재시도 나이(`DawnlineConsumerRetryStuck`)가 본다. 정지 중에도 이 값은 오르지만 부수 신호다. `>= 0` 은 exporter 가 커밋한 적 없는 파티션에 내는 `-1`(CLI 의 `-`)을 뺀다 — 모름을 합에 섞지 않는다 |
...
| `DawnlineKpiPromiseUnknown` | `dawnline_kpi_excluded{reason="promise_unknown"}` > 0 이 **30분 지속** | **초기값 — Phase 7 peak-day 에서 재검토.** 결과는 났는데 약속을 모르는 주문이 정시율에서 빠지고 있다 — 프로젝션 랙으로 설명되는 길이를 넘었으면 `fulfillment.planned` 가 오지 않고 있다 |
| `DawnlineRateLimitBypassed` | `dawnline_rate_limit_decisions_total{outcome="bypassed"}` 증가 — 닫힌 카운터, 미리 등록 | Redis 장애로 레이트 리밋이 꺼졌다 — 무인증 API 의 유일한 남용 방지 수단이 사라진 상태다. RB-03 |
| `DawnlineCancelTooLate` | `dawnline_cancel_too_late_total` 증가 — **열린 카운터**(camp), 부재를 다루는 식 | 배송이 끝난 주문에 취소가 도착했다 — 물리적 배송과 주문 상태가 어긋난 건이 생겼고 사람이 처리해야 한다. 자동 보상은 없다(§6.10) |
...
| 카오스 | Kafka/Redis 중단·복구, 인스턴스 강제 종료, DB 장애 | `make chaos-db` · `chaos-kafka` · `chaos-redis` · `chaos-kill`(`tools/chaos`, Compose `stop/start` · 계정 `NOLOGIN` · `SIGKILL`) — 끝에 같은 검증 표 V1–V7(`verify.sh`, 7-4 peak-day 도 같은 표), 알림은 실제 Prometheus 에 묻는다. PR 게이트가 아니라 수동 워크플로(`.github/workflows/chaos.yml`, 넷을 한 스택에서 차례로 · 보고를 아티팩트로) | 데이터 유실 0(V1 — 배차 불가는 **사유별**로 기대와 견준다) · 중복 0(V2 · V3) · DLQ 0(V5) · outbox 미발행 · 격리 0(V7) |
...
| 12 | 시간·난수는 주입 | 규칙 7 — 시계 쪽은 온전히 강제된다. 난수(`RandomGenerator`)는 아직 아니다 | 생성자 시그니처, seed 재현성 테스트, `libs/messaging` 이 저장 정밀도로 자른 `Clock` 빈을 제공. **규칙이 강제하는 것은 「주입받는다」이지 「테스트가 그 이음매를 쓴다」가 아니다** — `RateLimitApiIT` 는 주입된 시계를 쓰는 레이트 리밋을 **실제 시계로** 돌려, 러너가 느린 날에만 429 가 201 이 되는 형태로 2026-09-23 CI 에서 깨졌다(토큰 리필이 흘러간 벽시계만큼 일어난다). 고정 시계 빈을 넣어 닫았고, 같은 클래스의 `리필은_실제_시간이_흘러도_일어나지_않는다` 가 **벽시계를 1.2초 흘려 보내며** 그것을 지킨다. **SQL 의 DB 시계**(`now()` · `CURRENT_TIMESTAMP` …)는 ArchUnit 이 보지 못하는 문자열 안이라 `SqlUsesInjectedClockTest` 가 운영 소스의 리터럴 전부에서 뺀다(2026-09-26, ADR-066 결정 4 — 시뮬레이션 오프셋 아래에서 outbox 미발행 나이가 음수가 된 것이 계기) | 규칙 7 ✅(양방향) / SQL 시계 ✅(음성 표본 — 룰 `updated_at = now()` 를 되돌리면 빨강) / 「이음매를 쓰는가」는 ✗ — IT 마다 사람이 본다 |
...
| 11 | [ADR-060](adr/ADR-060-metrics-come-from-the-table.md) 결정 2 | 미터가 카탈로그를 지난다는 것 — 우회한 등록은 §9.1 대조 밖에 이름을 만들고, 게이지를 약한 참조로 되돌린다 |
...
| **규칙판** — 떠 있던 Prometheus 가 바뀐 규칙 파일을 다시 읽지 않았다. `make obs-check` 1 은 알림 **이름** 집합만 견줬고 이름은 그대로라 초록이었다 — **옛 규칙은 적재된 것처럼 보인다** (2026-09-25) | 7-3① 의 2차 chaos-db 앞 10분이 `DawnlineConsumerLag` 를 옛 식으로 판정했다. 같은 자리를 다시 보다가 둘째 원인이 나왔다(2026-09-26, 근거: 관측(재현됨)): `prometheus.yml` 은 **단일 파일** 바인드 마운트라 `git checkout` 이 파일을 새로 쓰면 컨테이너는 지워진 inode 를 붙들고(링크 수 0), 재적재가 「no such file」로 실패한다 — 그때는 다시 읽혀도 옛 규칙이 돈다 | **이름이 아니라 내용을 대조한다.** obs-check 1 이 그룹마다 (종류 · 이름 · 식 · `for` · 라벨 · 주석)의 해시를 파일과 적재된 것 양쪽에서 만든다 — 식은 Prometheus 의 `format_query` 로 양쪽 다 정규화한다. 마지막 재적재의 성공도 본다. 음성 표본 다섯(문턱 · `for` 만 · 주석 한 글자 · 파일은 원복됐는데 적재는 옛 것 · inode 만 바꾼 설정)이 빨갛다. 그래서 `make up` 이 설정만 바뀐 컨테이너를 다시 만들지 않는다는 사실은 문제가 아니라 **감지되는 조건**이 됐다 |
...
Phase 4 마감에 일곱째(검사 대상 집합)가, **Phase 5-0 에 여덟째(실행 순서)**, **Phase 5-1a 에 아홉째(환경이 결함을 가린다)**, **Phase 6 묶음 C 에 열째(빈 값끼리의 비교)와 열한째(기대값이 결함을 고정한다)**, **7-1 에 열두째(알림이 존재하지 않는 시계열을 본다)**, **7-2 에 열셋째(기본값이 기능을 조용히 끈다)와 열넷째(잘린 결과를 전체로 읽는다)**, **7-3 에 열다섯째(관찰이 처리를 바꾼다)** 가 붙었다. 7-3 뒤에는 새 축이 아니라 아홉째에 ④(1차 캐시가 잠근 행을 가린다)가 붙었다.
...
| 4 | **플래너 통계** | 통계 없는 테이블에서 플래너가 *짐작으로* 인덱스를 골랐다 — 50행에서 순차 스캔이 옳다 | CI 에서 autoanalyze 가 먼저 돌아. **변종 — 통계는 있는데 분포가 틀렸다** (2026-09-25, #65 의 첫 CI): `rm_orders` 걸린 행 셈의 픽스처는 전부 배송 결과를 갖고 있었고, 그래서 「결과 없음」의 선택도가 0 으로 잡혀 generic 계획이 순차 스캔을 골랐다 — 로컬에서는 단독으로도 초록이었고 CI 에서만 같은 DB 의 다른 행이 통계를 움직였다. 「통계가 있다」는 첫 어설션은 필요하지만 충분하지 않다: **통계는 픽스처의 분포를 요약하고**, 그 분포가 운영과 다르면 계획은 다른 질문에 답한다. 고친 것은 픽스처(운영 분포 — 오늘치는 결과가 없다)와 검사 범위(하루 한 번 도는 문장은 도는 계획인 custom 만 — `docs/benchmarks/phase7-retention-indexes.md` §3)다 **대응의 다른 모양 — 가드를 계획 선택이 아니라 문장 구조에 건다** (2026-09-25, [ADR-061](adr/ADR-061-unfinished-work-has-no-window.md)): 계획 선택은 규모 · 통계 · 분포에 따라 갈리지만, 문장이 **어떤 표를 읽는가**는 작은 픽스처에서도 참 · 거짓이 갈린다. `dawnline_routes` 의 첫 판(#68)은 `EXISTS` 로 되돌리면 해시 서브플랜이 `rm_orders` 전체를 읽는 회귀를 운영 크기에서만 드러냈다 — 이 축의 전형이다. 완료를 쓰기 때 적어 질의가 `rm_orders` 를 읽지 않게 되자 `KpiViewsIT` 가 「계획에 `rm_orders` 가 없다」를 볼 수 있게 됐고, 이 축의 어느 변종(통계 없음 · 분포 틀림)도 그 검사를 흔들지 못한다. **가드를 걸 수 있는 자리가 둘이면 규모와 무관한 쪽에 건다** |
...
| 9 | **환경이 결함을 가린다** | 검사가 보려는 성질을 **환경이 기본값으로 만족**시키고 있었다. 둘은 같은 얼굴이다 — ① Phase 1: 개발 기계의 `Clock.systemUTC()` 가 나노초를 내지 않아 저장 정밀도(마이크로초) 불일치가 숨어 있었다 ② Phase 5-1a: 컨테이너 세션이 UTC 라 파티션 경계 검사가 **함수가 세션 존을 써도 그대로 통과**했다 | **환경을 일부러 어긋나게 만들어 드러낸다** (2026-09-19): 같은 커넥션에서 `SET TIME ZONE 'Asia/Seoul'` 로 만들었더니 경계가 `FROM ('2035-05-09 15:00:00+00')` 로 나와 검사가 실패했다. Phase 1 쪽의 대응은 저장 정밀도로 자른 `Clock` 빈을 `libs/messaging` 한 곳에 둔 것이다 — 양쪽 다 **기본값이 맞춰 주던 것을 검사가 직접 말하게** 하는 형태다. **반대 방향도 있다**: 환경이 바뀌어 결함이 *사라진* 경우다 — [ADR-009](adr/ADR-009-url-path-api-versioning.md) 결정 3 의 음성 표본(`/actuator/health` 의 세그먼트를 버전으로 파싱해 프로브가 깨진다)은 Boot 4.1.x 에서 재현되지 않는다(2026-09-19). 그때 남는 것은 **초록인 채로 아무 말도 하지 않는 검사**이고, 이쪽의 대응은 결정을 방어적으로 유지하되 그것을 지킨다고 *말하던* 줄의 범위를 좁히는 것이다 **③ 설정 둘의 관계를 기본값이 만족하고 있었다** (2026-09-25, 7-3): 끝없는 재시도의 한 바퀴는 폴 → 리스너 실패 → 백오프라, 백오프 상한이 `max.poll.interval.ms` 보다 길면 컨슈머가 그룹에서 쫓겨나고 돌아와 같은 레코드로 또 쫓겨난다 — 끝없는 재시도가 끝없는 리밸런스가 된다. 운영 기본값(5초 · 300초)이 그 관계를 **우연히** 만족하고 있었고 아무것도 그것을 요구하지 않았다. `ConsumerRetryIT` 의 음성 표본(5초 · 3초)에서 멤버 id 가 바뀌어 관측이 됐다(근거: 관측(재현됨)). IT 의 관측으로 끝내지 않고 **기동에서 거부한다** — 보존 기간의 순서 검증(dispatch 의 후보 ≤ 계획 보존, fulfillment 의 주문 ≤ 웨이브 보존)과 같은 형태다(`DawnlineErrorHandlers.requireBackOffWithinPollInterval`). **두 설정이 서로를 전제하면 그 관계는 한 설정의 기본값이 아니라 기동 검사가 지킨다** **④ 1차 캐시가 잠근 행을 가린다 — ORM 판** (2026-09-26, [ADR-025](adr/ADR-025-wave-admission-share-lock.md) 후속 정정): 잠금 SQL(`FOR SHARE` · `FOR UPDATE`)은 맞았고 락도 걸렸다 — 그런데 그 질의를 엔티티로 매핑하면 Hibernate 는 같은 트랜잭션이 잠그기 **전에** 읽어 둔 영속성 컨텍스트의 사본을 돌려준다. 잠근 행을 읽었다고 믿었지만 본 것은 기다리기 전의 상태였고, 편입은 그렇게 **닫힌 웨이브에 주문을 넣었다**. 순차 테스트에서는 잠그기 전과 후의 행이 같아서 차이가 없다 — **경합이 없는 환경이 결함을 가렸다.** 드러낸 것은 CI 의 경합 테스트(`WaveLifecycleIT`)가 가끔 그 창에 닿은 것이고(운영자 쪽 `OptimisticLockException`), 고정한 것은 인터리빙을 `pg_stat_activity` 로 세운 `WaveLockedReadIT` 다(근거: 관측(재현됨)). 대응은 ③ 과 같은 형태다 — 캐시가 맞춰 주던 것을 코드가 직접 말하게: 잠금 질의는 버전을 읽고 다르면 다시 읽는다(CLAUDE.md 코딩 컨벤션) |
| 10 | **비교 대상이 비어 있다** | 두 결과가 같은지를 보는 검사에서 **비교하는 칸이 양쪽 다 비어 있었다** — `null` 과 `null` 은 같으므로 검사는 참이었다. `ProjectionShuffleIT` 는 같은 사실 묶음을 두 순서로 투영해 칸마다 같은지 본다. 칸 enum 과 DDL 의 대조는 「이 칸이 비교 대상이다」만 말했고, **시나리오가 그 칸을 채우는지**는 아무도 말하지 않았다 | 음성 표본으로 드러났다 (2026-09-24, Phase 6 묶음 C): `rm_waves.camp_code` 의 투영을 지워도 이 IT 가 초록이었다 — 두 순서가 똑같이 `null` 을 남겼기 때문이다. **비교의 전제는 비교 대상이 채워졌다는 것이다.** 이제 두 비교 테스트는 「기준 행이 모든 칸을 한 번은 채운다」를 첫 어설션으로 말하고, 뺄 칸은 이유와 함께 `NULL_IN_SCENARIO` 에 적는다(빼는 방식 — 지금 비어 있다). 같은 음성 표본이 이제 두 비교 테스트에서 빨갛다. 이 축이 다음에 나올 자리는 **같음을 보는 모든 검사**다 — 스냅샷 비교, 재생 결과 비교, 왕복(round trip) 검사. 그 입력이 비교 칸을 채우는지를 먼저 묻지 않으면 빈 칸은 조용히 통과한다. **변종 — 비교할 행이 이미 지워져 있다** (2026-09-25): 걸린 행 셈이 삭제의 여집합인지를 **삭제 뒤에** 보면, 셈이 종결 행까지 넓어지는 결함은 보이지 않는다 — 넓어진 셈이 더 셀 행을 삭제가 먼저 치웠다. `ReadModelRetentionIT` 의 첫 형태가 셈에서 `delivery_outcome IS NULL` 을 지워도 초록이었다. 이제 두 여집합 검사(`rm_orders` · `fulfillment_orders`)는 **지우기 전에** 같은 행을 센다([ADR-058](adr/ADR-058-shipment-and-read-model-retention.md) 「결과」) **변종 — 검사 대상이 사라져 판정 불가가 통과로 보인다** (2026-09-25, 7-0c): 정리기의 「걸린 셈」 게이지가 「세기 전에는 `NaN`」인지를 보는 테스트는, **정리기가 GC 되어도** `NaN` 을 봤다 — Micrometer 는 게이지의 상태 객체를 약한 참조로 들고, 대상이 사라진 게이지가 내는 값이 바로 `NaN` 이다. 검사가 기다리던 값과 대상이 없어졌을 때의 값이 같아서, **대상이 사라져도 검사는 통과했다**. 이 저장소의 규약 「`NaN` 은 모름 — 0 은 주장이다」(§9.1)가 여기서는 역으로 결함을 숨겼다: 「모름」을 값으로 쓰면, 모름이 **다른 원인**(GC · 미등록 · 다른 레지스트리)에서도 같은 값으로 나온다. 드러난 것은 `DispatchRetentionIT` 가 셈 뒤의 값 1 을 기다리다 `NaN` 을 받았을 때다(근거: 관측(재현됨)). 그 PR 은 테스트가 정리기를 필드로 잡게 했는데, 그것은 테스트의 우회다 — 같은 결함이 운영에서도 가능하다(스프링 빈은 컨텍스트가 잡지만 람다 · 임시 객체를 대상으로 한 게이지는 아니다). **닫은 곳은 등록 방식이다**: 게이지는 `DawnlineMeters` 한 곳에서 `strongReference(true)` 로 등록한다([ADR-060](adr/ADR-060-metrics-come-from-the-table.md) 결정 2, ArchUnit 규칙 11). 그 짝으로 헬퍼의 테스트가 **대상의 약한 탐침이 GC 뒤에도 살아 있는가**를 본다 — `NaN` 이 아니라 원인을 본다. 규칙: **「모름」을 기대값으로 쓰는 검사는, 그 값이 검사 대상 밖의 원인으로도 나오지 않는다는 것을 먼저 세운다.** **셸 판 — 빈 집합끼리의 ✅** (2026-09-25, 7-3): 카오스의 검증 표 V1(주문 = 후보 + 취소 + 배차 불가 · 빠진 주문 0)을 기준 직후에 재자 `0 = 0 + 0 + 0` 으로 ✅ 였다 — T0 이후 주문이 없으면 「빠진 주문 0」은 빈 집합끼리의 비교다. 이제 V1 은 전제를 먼저 말한다(`--expect-orders N`, 없으면 주문 ≥ 1). 같은 스크립트에서 이웃한 결함이 하나 더 나왔다: 판정을 `$(…)` 안에서 모아 **✗ 가 있어도 종료 코드 0** 이었다 — 장애 중의 표는 빠진 주문 1,200 을 정확히 봤는데(✗), 호출한 카오스는 그것을 「검사가 유실을 못 봤다」로 읽었다. 판정이 서브셸 밖에서 모이고, 음성 표본(틀린 전제 · 틀린 DLQ 기대 → 1)이 그것을 본다 |
| 11 | **기대값이 결함을 고정한다** | 테스트가 **출력을 그대로 기대값으로 적었다** — 그래서 출력의 결함이 테스트에 박제됐다. ops-web 의 `shortId` 는 UUID 의 **앞** 8자를 보였고, 대시보드 테스트는 그 출력 `'0199c000 · 웨이브 1'` 을 기대값으로 고정했다. 픽스처의 두 라우트 id 는 이미 앞 8자가 같았지만, **「이름이 서로 갈린다」를 묻는 테스트가 없었다** — 검사는 「이름이 된다」를 보는 척하며 결함 있는 이름을 정확히 확인하고 있었다. 10번과 다른 변종이다: 저쪽은 비교가 **아무것도 보지 않았고**, 이쪽은 비교가 **틀린 것을 정확히 봤다**. 원인은 형식이 아니라 사실 하나다 — **UUIDv7 의 앞 48비트는 밀리초 시각**이고(불변규칙 10) 앞 8자는 그 윗 32비트다 — 2¹⁶ ms(약 65초)마다 한 번 바뀌므로 **한 계획의 라우트처럼 가까운 때 만든 id 는 앞 8자가 같다** | Compose 스모크의 데모 출력이 처음 드러냈다 (2026-09-24, Phase 6 묶음 C3): 재배정 결과가 「`01a0d373 → 01a0d373`」 — 옮긴 것을 보이지 못했다. 화면에서는 재배정 대상 목록의 두 라우트가 같은 이름이었다. 고친 것은 **뒤** 8자(난수 부분)이고, 그 짝으로 **성질을 묻는 테스트**를 뒀다(`RouteMap.test.tsx` 「같은 순간에 만든 라우트도 이름이 갈린다」 — 전제로 두 id 의 앞 8자가 같은지를 먼저 확인한다). 앞 8자로 되돌리면 그 테스트와 대시보드 테스트가 함께 빨갛다. **규칙: 기대값은 출력이 아니라 명세에서 온다.** 출력을 기대값으로 적을 때는 그 값이 만족해야 할 성질(여기서는 「이름이 서로 갈린다」)을 묻는 테스트가 짝으로 있어야 한다. 원인을 여기 적는 이유는 다음 사람이 `shortId` 를 앞으로 되돌릴 이유가 「읽기 좋아서」일 것이기 때문이다 — 앞 8자는 시각이라 한 화면의 id 들이 서로 닮는다 |
| 12 | **알림이 존재하지 않는 시계열을 본다** | 알림 식은 이름으로 시계열을 고른다 — 고른 것이 비어 있으면 식의 결과도 비고, **빈 결과는 「울리지 않음」과 구별되지 않는다.** §9.4 의 「계획 시간 p95 > 45s」는 `dawnline_plan_duration_seconds_bucket` 을 읽는데, 그 버킷은 **한 번도 나온 적이 없었다**: 히스토그램을 켜는 속성 키가 Prometheus 이름이었고 Spring 은 미터 이름으로 맞춘다. 속성 파일의 검사(`ObservabilityResourcesTest`)는 「그 문자열이 있는가」를 봤다 — 켜려던 것이 켜졌는지가 아니라. 알림은 규칙 파일에 있었고, 규칙은 문법이 맞았고, 읽을 것만 없었다 | 대조가 처음 돌며 드러났다 (2026-09-25, 7-1, 근거: 관측(재현됨) — 같은 타이머를 두 키로 등록해 긁었더니 앞의 키에서는 `_bucket` 이 없었다. `http_server_requests_seconds_bucket` 도 없었다). 모양이 둘이고 닫는 자리도 둘이다. **카운터** — 라벨이 닫힌 카운터가 첫 사건에 태어나면 `increase` 가 그 사건을 놓친다(7-0 A7) — 는 `AlertedCountersContract` 가 닫는다: 다섯 서비스의 IT 가 「알림 걸린 닫힌 카운터의 조합이 기동 때 전부 있다」를 본다. **히스토그램 · 플랫폼 지표** — 켜는 자리가 코드가 아니라 속성 파일이다 — 는 Compose 스모크의 `make obs-check` 「긁히는 이름」이 닫는다: 실제 스택을 긁어 `_bucket` 이 있는지를 본다. 같은 PR 의 다른 발견이 이 축의 이웃이다: §9.1 이 닫힌 라벨 값을 다섯 자리에서 빠뜨리고 있었다(`action` 의 `CLOSE_WAVE` · `REQUEUE_OUTBOX`, `cause` 의 `unknown`, `tier` 의 `NEXT_DAY`, geo `index` 의 `zone` · `wave_lock`) — 표가 모르는 값으로 식을 쓰면 그 식도 없는 시계열을 본다. 카탈로그가 **라벨 집합**을 넷째 칸으로 들게 한 이유가 첫 실행에서 증명된 사례다([ADR-060](adr/ADR-060-metrics-come-from-the-table.md)). 규칙: **알림과 패널은 읽을 시계열이 실제로 긁힌다는 것을 검사로 세운다 — 이름이 적혀 있다는 것으로는 부족하다.** |
| 13 | **기본값이 기능을 조용히 끈다** | 기능을 켜는 자리가 코드가 아니라 **기본값**이었다 — 그 기본값이 「꺼짐」이면 기능은 설계서와 코드 양쪽에 있으면서 한 번도 돌지 않는다. outbox 의 `traceparent` 가 그랬다: §9.2 가 적고 Phase 2 가 칸을 두었고, 제공자 인터페이스의 문서는 「`libs/observability` 가 등록하면 그쪽이 이긴다」고 했다. 등록은 없었고 기본값 `NONE` 이 이겼다 — outbox 를 지나는 모든 이벤트가 트레이스 없이 나갔다. 그 자리를 보는 검사가 없었으므로 아무것도 실패하지 않았다. 위 「꺼 둔 검증」의 **운영 코드판**이다: 없는 job 이 실패하지 않듯 꺼진 기능도 실패하지 않는다. **변종 — 설정 키가 주석이 말하는 것보다 많이 끈다**: lean 구성(`docker-compose.lean.yml`)은 「스팬 수집은 하되 OTLP 전송을 안 한다」고 적고 `management.tracing.export.enabled=false` 를 썼는데, Boot 4.1 은 그 키로 W3C 전파기까지 no-op 으로 바꾼다 — lean 스택은 서비스마다 트레이스를 끊고 있었다. 기본값이 아니라 **키의 범위**가 주석과 달랐다 | 7-2 의 첫 검사(`OutboxTraceparentIT`)가 드러냈다 (2026-09-25, 근거: 관측(재현됨) — 행의 `traceparent` 가 비어 있었고, 제공자를 넣자 이번에는 발행된 traceId 가 달랐다: 템플릿 관측이 헤더를 릴레이 폴링의 트레이스로 다시 썼다. 변종은 그 검사를 `export.enabled=false` 로 켰을 때 전파기의 `fields()` 가 빈 목록인 것으로 보였다). 닫은 방식: 기본값을 **클래스 조건으로만** 고른다(트레이싱 클래스가 있으면 진짜 구현 — 빈 등록 순서에 기대는 `@ConditionalOnBean` 을 쓰지 않는다), 그리고 검사가 **켜졌다는 것을 전제로 말한다** — 「행에 traceparent 가 있다」가 첫 어설션이고 음성 표본이 제공자를 `NONE` 으로 돌려 그 어설션이 빨개지는 것을 보였다. lean 은 익스포터 키(`export.otlp.enabled`)만 끈다([ADR-062](adr/ADR-062-trace-survives-the-outbox.md)). 규칙: **기능을 켜는 자리가 기본값이면, 그 기능이 켜졌다는 것을 보는 검사가 있어야 한다 — 설정의 뜻은 주석이 아니라 그 검사가 정한다.** |
| 14 | **잘린 결과를 전체로 읽는다** | 검사가 **상한이 있는 조회**(검색 한도 · 페이지 · 배치)의 결과를 집합 전체로 읽었다 — 상한에 닿은 결과에서 「없다」를 결론 냈다. Compose 스모크의 트레이스 검사(`make obs-check` 4)는 TraceQL 검색을 `limit=100` 으로 물었고, 데모 웨이브 하나의 트레이스는 177개였다 — 주문 176 · 계획 1. tracking 을 지나는 것은 계획 트레이스 하나뿐이라, **그 하나가 잘린 100개 안에 드는지가 순서의 우연**이었다. 그러면 초록도 빨강도 답이 아니다: 초록은 「계획 트레이스가 운 좋게 들어왔다」, 빨강은 「tracking 이 끊겼다」와 「잘렸다」를 구별하지 못한다. 10번(비교 대상이 비어 있다)의 이웃이다 — 저쪽은 비교할 값이 비었고, 이쪽은 **비교할 집합이 모자랐는데 모자란 줄 몰랐다** | 음성 표본을 만들다 드러났다 (2026-09-25, 7-2, 근거: 관측(재현됨)): tracking 의 리스너 관측을 끈 스택에서 검사가 빨갰는데, 그 결과가 **정확히 100개**였다 — 한도와 같은 수는 「그게 전부」가 아니라 「더 있을 수 있다」다. 같은 웨이브를 한도 1000 으로 다시 물어 177개를 받고서야 빨강이 tracking 의 결손이라고 말할 수 있었다. 고친 것은 둘이다: 한도를 올렸고(1000), **결과가 한도에 닿으면 실패한다** — 한도를 올리는 것만으로는 같은 결함이 규모가 커지는 날로 미뤄질 뿐이다. 저장소를 훑어 상한 있는 조회를 쓰는 다른 검사를 찾았다: 나머지는 전부 「하나를 고른다」(`LIMIT 1`)이거나 **찾으려는 것이 없으면 실패하는** 모양이다(`DlqReplayIT` 의 `recordAt` — 잘리면 빨강이지 초록이 아니다). 잘린 결과에서 **부재를 결론 내는** 자리는 이 하나였다. 규칙: **상한이 있는 조회를 검사에 쓰면, 상한에 닿은 결과는 결과가 아니라 「모름」이다 — 검사는 그때 실패한다.** 부재나 합집합처럼 전체를 요구하는 결론은 전체를 받았다는 것을 먼저 세운다 |
| 15 | **관찰이 처리를 바꾼다** | 관찰 코드가 **자기가 걸린 훅의 계약**을 짐작했다 — 재시도 관찰자(`ConsumerRetryObserver`)는 `RetryListener.failedDelivery` 에 재시도 목록 안의 실패만 온다고 보고 그 예외의 행을 닫힌 라벨 `reason` 으로 셌다. 관찰자의 단위 테스트는 그 가정 위에서 초록이었다 — 즉시 DLQ 로 가는 실패를 **실제 핸들러에 꽂은 관찰자**에 준 적이 없었다 | 전체 빌드의 이웃 IT 가 드러냈다 (2026-09-25, 7-3): Spring Kafka 는 즉시 DLQ 로 갈 실패도 `failedDelivery` 로 한 번 알린다. 관찰자는 목록 밖의 값 `deserialization` 으로 카운터를 올리다 던졌고, **그 예외가 에러 핸들러의 복구를 막아** 깨진 봉투가 DLQ 에 가지 못하고 되풀이됐다 — ops-api 의 `DlqReplayIT` 셋이 30초를 기다리다 실패했다(근거: 관측(재현됨)). 지표가 틀리는 것은 보이는 결함이고, **처리가 바뀌는 것은 장애다.** 두 겹으로 닫았다: 그 행은 세지 않는다 · 관찰 콜백의 예외는 경고로 삼킨다. 회귀 테스트는 실제 관찰자를 꽂은 핸들러에 역직렬화 실패를 주고, 음성 표본은 **두 겹 중 하나만 빼면 초록 · 둘 다 빼야 빨강**이었다 — 방어가 정말 두 겹이라는 확인이다. 규칙: **관측은 절대 처리를 막지 않는다** — 관찰 코드는 자기 훅이 처리 경로 위에 있으면 그 예외를 처리 경로로 올리지 않고, 그 훅의 계약은 짐작이 아니라 실제 호출자로 테스트한다 |
...
| 059 | **dispatch 의 보존은 계획 단위다 — 나이는 `finished_at`, 후보는 계획이 끝나야 지운다** — 나이의 칸은 계획의 `finished_at` 하나(**안전한 쪽이 아니다** — 계획 뒤의 사실이 배송일 안에 끝난다는 전제로 최대 하루 이르다, 계열의 여유 90 − 1 − 30 = 59일) · 종결 = `PUBLISHED`·`FAILED` ∧ 그 계획의 모든 stop 이 종결 목록 안(모름은 종결이 아니다) · 후보·설명은 **계획이 종결일 때만** 30일, 계획 단위 · 재배정·재계획·취소의 후보 조인 넷은 후보가 없으면 **`candidates-expired` 409**(내부 조인이 stop 을 조용히 빼고 출발 라우트를 비웠다 — 근거: 관측(재현됨), `DispatchRetentionIT`) · 끝나지 않은 계획은 남기되 센다(`dawnline_route_plans_stuck`, 임계 30일 — 처음 붙잡히는 자리) · 상한 365일(계열째, 계획 없는 웨이브의 후보) · 계획 하나를 한 트랜잭션에서 자식부터 — 새 인덱스 없음, 트랜잭션 상한은 「계획 하나」(`peak` 평상시 2.4만 · 최악 5.4만 행) · 계열 네 표는 설정 키 하나, 성공 나이 여섯 라벨이 함께 움직인다 | `routes.updated_at` 신설(stop 전이마다 라우트 쓰기, 피크 673건/초), 후보를 자기 `updated_at` 30일로(stop 이 조용히 빠진다), 「계획이 30일 지났다」만으로(끝나지 않은 라우트의 후보), 가드 없이 조건만(조건은 고른 결과지 강제가 아니다), 표마다 `ctid` 배치 + `NOT EXISTS`(가드 넷, 1,350만 행), `ON DELETE CASCADE`(순서가 스키마에 숨는다), 걸린 계획을 90일로(30–90일이 셈 밖) | [ADR-059](adr/ADR-059-dispatch-retention-is-per-plan.md) |
| 060 | **메트릭은 표에서 온다** — §9.1 의 행마다 카탈로그 항목 하나(`DawnlineMetrics`: Prometheus 이름 · 타입 · Micrometer 이름 · **라벨 집합** — 닫힘/열림), 표 ↔ 카탈로그 양방향 대조 · 등록은 `DawnlineMeters` 한 곳(타입 · 라벨 키 · 닫힌 값을 등록 때 대조, `histogram` 은 버킷, **게이지는 강한 참조** — 약한 참조의 `NaN` 이 「모름」 검사를 대상 없이 통과시켰다, 근거: 관측(재현됨)), 우회는 ArchUnit 규칙 11 · 알림 걸린 카운터는 닫혔으면 미리 등록, **열렸으면 식이 부재를 다룬다**(`increase(x[w]) > 0 or (x unless x offset w)` — 1 로 태어난 시계열의 첫 증가를 `increase()` 가 못 읽는다, 근거: 관측(재현됨), `PrometheusRulesIT`) · 대조가 처음 돌며 찾은 것: 히스토그램 키가 미터 이름과 달라 버킷이 없었다(HTTP 도), §9.1 이 닫힌 값 다섯 자리를 몰랐다(action 둘 · cause `unknown` · tier `NEXT_DAY` · `geo_lookups` 의 index 둘), §9.4 가 그리라고 적은 「실패 · 라우트 진행」에 행이 없었다(`dawnline_kpi_delivery` · `dawnline_routes` 를 더했다) — 앞의 둘은 **알림이 존재하지 않는 시계열을 본다**(§13 축 12) | [ADR-060](adr/ADR-060-metrics-come-from-the-table.md) |
| 061 | **끝나지 않은 일에는 창이 없다 — 라우트의 완료는 쓰기 때 다시 센다** — `rm_routes.completed_at` · `live_count`(V8)는 `completed_count` · `failed_count` 와 같은 재집계가 쓰는 집계 칸(ADR-051 결정 4 의 행 단위), 값은 사실에서(마지막 결과 시각) · **`live_count = 0` 은 다섯째 상태 `void`** — 할 일이 없는 라우트에 완료 시각을 만들지 않는다 · 다시 세는 사실은 `route.assigned` · `delivery.status` · **`order.cancelled`(새로)** · `dawnline_routes` 의 `assigned` · `in_progress` · `unknown` 은 창 없음, `completed` · `void` 는 KPI 창 · 질의는 `rm_routes` 만 읽는다(운영 크기 7–11 ms, **인덱스 없음** — 부분 인덱스는 창 쪽 순차 스캔 때문에 값을 하지 않았다) · #68 의 창이 창 밖 진행 중 55 라우트를 뺐고, 창만 걷으면 매분 0.9 초 · 1.7 GB 였다 | [ADR-061](adr/ADR-061-unfinished-work-has-no-window.md) |
