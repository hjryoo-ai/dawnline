...
| 복구 뒤 | 30초 안에 전부 처리 — 1,200 = 후보 1,023 + 배차 불가 177, 빠진 주문 0(검증 표 V1). **177 은 장애 탓이 아니다** — 사유별로 `OUT_OF_STOCK` 1(시드의 재고 예외 `SKU-00666` × 2) + `MAX_PUSHES_EXCEEDED` 176. 176 은 캠프 하나의 `DAWN` 주문 전부이고, 그 캠프의 `DAWN` 웨이브 넷(9/25–9/28)을 같은 볼륨의 앞선 데모 넷이 조기 마감해 두었다(07:43 · 07:47 · 07:52 · 08:20 UTC). 원래 컷오프와 밀린 셋이 모두 닫혀 있었다(ADR-063). 2차는 15:00 UTC 뒤라 원래 컷오프가 하루 밀렸고 셋째 밀림이 열린 9/29 에 닿았다 — 그래서 1. **합 하나는 이 차이를 삼켰다**: 그 뒤 검증 표는 배차 불가를 사유별로 낸다(`V1·사유` — 흐름 사유는 0, 이 실행에서라면 ✗) |
