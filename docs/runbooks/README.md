...
| `DawnlineCancelTooLate` | **메트릭** `max by (client_id) (kafka_consumer_fetch_manager_records_lag_max{service="order-service"})` — order-service 가 `order.dispatched` · `delivery.status` 를 늦게 알았나 | 랙이 있다 → 주문 쪽이 배송을 모른 채 취소를 받았다 — 랙이 원인이다(RB-01 §2) · 랙이 없다 → 고객이 배송 직후에 눌렀다 — 경합 창의 끝이고 결함이 아니다. **어느 쪽이든 그 건은 사람이 닫는다**: 물건은 배송됐고 주문은 취소다(§6.10 — 자동 보상 없음). 건 목록은 ops-api `GET /api/v1/camps/{campId}/exceptions`(취소됐는데 배송된 주문, 창 없이 전부) · 주문 id 는 dispatch 로그 「배송이 끝난 뒤 도착한 취소입니다」 | — |
