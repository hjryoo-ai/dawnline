...
| 관련 설계 | §5.5(커맨드 위임 — 결과 넷, 상관 헤더) · §9.3(`auditId`) · §9.5 · ADR-052 재검토 지점 4 · ADR-054 |

**먼저 본다 — SQL** 해소할 행 — 행의 `id` 가 코어 로그의 `auditId` 다:

```bash
sql ops "SELECT id, action, target_type, target_id, actor, request, result, created_at
           FROM audit_logs
          WHERE (result = 'UNKNOWN' OR (result = 'PENDING' AND created_at < now() - interval '5 minutes'))
            AND action <> 'DLQ_REPLAY'
          ORDER BY created_at"
```
...
코어는 ops-api 가 싣는 `X-Dawnline-Audit-Id` 를 MDC `auditId` 로 남긴다(§9.3) — **그러나 그 id 를 싣는 줄은 코어가 무언가를 로그할 때만
생긴다.** 코어는 커맨드의 **성공**만 INFO 로 남긴다:

| 커맨드 | 성공의 줄(`auditId` 를 싣는다) | 거절(4xx) · 멱등 무동작 |
|---|---|---|
| `REASSIGN_STOP` | dispatch 「stop 재배정: orderId=…」 | 줄 없음 |
| `CANCEL_ORDER` | order 「주문을 취소했습니다. orderId=…」 | 줄 없음 |
| `RUN_PLAN` | **없다** — 재실행의 성공도 INFO 줄을 남기지 않는다 | `ALREADY_PUBLISHED` 는 DEBUG |
| (참고) `CLOSE_WAVE` · `REQUEUE_OUTBOX` | 「웨이브를 수동 마감했습니다」 · 「outbox 격리를 풀었습니다」 | 줄 없음 — 그래서 1 의 다시 누르기가 먼저다 |

```bash
logs dispatch-service 24h | grep '"auditId":"<id>"'      # REASSIGN_STOP
logs order-service 24h    | grep '"auditId":"<id>"'      # CANCEL_ORDER
```

**줄이 있으면 적용됐다 — 줄이 없다는 것은 아무것도 말하지 않는다.** 요청이 닿지 않았을 수도, 닿아서 거절됐을 수도, `RUN_PLAN` 처럼 줄을
남기지 않고 적용됐을 수도 있다(관측: 2026-09-25 로컬에서 발행된 웨이브에 `RUN_PLAN` 을 보냈다 — 응답 200 `ALREADY_PUBLISHED`, 감사
`SUCCEEDED`, dispatch 로그에 그 `auditId` 를 싣는 줄 0). §9.5 의 「요청이 닿은 흔적이 없으면 `FAILED`」는 흔적의 부재를 근거로 읽는다 —
그 문장을 이 런북이 정정한다: **줄이 없으면 코어의 현재 상태로 간다.**

**코어의 현재 상태** — 줄이 없을 때의 근거다:
...
앞의 행을 판정대로 바꾼다. **ops 계정으로, 한 행씩, 지금 값이 `UNKNOWN` · `PENDING` 인 행만**:

```bash
dc exec -T postgres psql -U dawnline_ops -d dawnline_ops -c \
  "UPDATE audit_logs SET result = '<SUCCEEDED|FAILED>' WHERE id = '<id>' AND result IN ('UNKNOWN', 'PENDING')"
```

- **무엇을 근거로 닫았는지 남긴다** — 다시 누른 행의 id, 로그 줄, 현재 상태 질의 중 무엇이었는지. **`audit_logs` 에는 그 칸이 없다** — 지금은 사건
  기록(티켓 · 포스트모템)에 남긴다. `request` 에 적지 않는다 — 그 칸은 커맨드의 인자만이다(§5.5).
- 카운터(`dawnline_ops_commands_total`)는 되돌리지 않는다 — 그것은 「그때 모른다고 적었다」의 수이고, 그 자체가 사실이다.
- 이 일을 코드로 옮기는 것(ops-api 가 코어 상태를 다시 읽어 닫기)은 `UNKNOWN` 이 실제로 쌓이면 연다 — ADR-052 재검토 지점 4.

## 참조

- `docs/DESIGN.md` §5.5 · §9.3 · §9.5
- [ADR-052](../adr/ADR-052-delegation-client-is-generated-from-the-committed-contract.md) · [ADR-054](../adr/ADR-054-early-wave-close-is-an-operator-cutoff.md)
