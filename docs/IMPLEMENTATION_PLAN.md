...
| `cancel_too_late_total{camp}` 알림식 | 7-1 의 규칙 파일 | §9.1, Phase 7-1 (a) — ✅ 닫았다(ADR-060, 규칙 식이 부재를 다룬다 · DLQ 도 같은 모양이었다) |
...
**작업** (순서 2026-09-25 확정 — 7-0 → 7-0b → 7-0c → 7-1 → 7-2 → 7-3 → 7-4a → 7-4 → 7-5 → 7-6 → 7-7. **같은 날 정정**: 7-5 런북을 7-2 뒤에 먼저 했고, 그 뒤는 7-5b(차례 · 소음) → 7-3(소비 측 분류 포함) → 7-3b(감사 해소 행) → 7-4a → 7-4 → 포스트모템 → 7-6 → 7-7)
...
1. Grafana 대시보드 4종 JSON, Prometheus 알림 규칙(§9.4) 커밋.
   ...
   **뒤의 셋은 한 패널에 겹쳐 놓는다** — 쌍이 *갈리는 것*이 정보이기 때문이다(§9.1):
   ...
   장애 중에 그 일은 일어나지 않는다.
   **→ [ADR-060](adr/ADR-060-metrics-come-from-the-table.md)** (2026-09-25 승인 — **메트릭은 표에서 온다**: 카탈로그 항목의 칸 넷
   (이름 · 타입 · Micrometer 이름 · 라벨 집합 닫힘/열림), 등록은 `DawnlineMeters` 한 곳(게이지는 `strongReference(true)` —
   7-0c 의 「정리기를 필드로 잡기」를 테스트가 아니라 등록 방식에서 닫았다), 우회는 ArchUnit 규칙 11, 알림 걸린 카운터는
...
3. 카오스 스크립트: `make chaos-kafka`, `make chaos-redis`, `make chaos-kill dispatch`. 각 실행 후 검증 SQL(주문 수 = 후보 수 + 취소 수, 라우트 stop 주문 중복 0, processed_events 중복 0)을 자동 실행.
   ...
   **소비 측 오류 분류가 여기서 들어온다**(2026-09-25 결정, §8.4 · RB-02 §3): DLQ 는 독약 메시지의 자리이지 장애의 자리가 아니다.
   ...
   **첫 워크플로 실행 (2026-09-26, run 36212841995 — ✗)**: `redis` · `kill` ✅, `db` · `kafka` ✗. 셋을 찾았다 —
   ① `chaos-db` 의 smoke 가 비-201 을 받아 중간에 끝났는데 **원인을 볼 수 없었다**: smoke 로그는 뒤 단계가 같은 이름으로 덮었고, 로그 아티팩트의
   `docker compose logs` 는 프로필(app · obs) 없이 불려 인프라 넷만 담았다 — `ci.yml` 의 Compose 스모크도 만든 뒤로 늘 그랬다(근거: 관측(재현됨) —
   로컬에서 같은 명령이 컨테이너 넷, 프로필을 주면 열넷). ② 그 밀림이 `chaos-kafka` 의 V1 을 오염시켰다 — 하류 집합을 자기 `created_at >= T0` 로만
   걸러 T0 전 주문이 끼었다(`OUT_OF_STOCK` 1 ↔ 2). 로컬 재현: 옛 식은 `3006 = 3607 + 0 + 3 ✅`(등식이 거짓인데 초록), 새 식은 `3006 = 3003 + 0 + 3`.
   고친 것: 로그 수집에 프로필 · env 파일 · 시각 · smoke 로그는 카오스마다 제 이름 · V1 의 하류 집합은 T0 주문 집합과 교집합. **`chaos-db` smoke 실패의
   원인은 아직 모른다** — 다음 실행이 그것을 볼 수 있게 한 것이 이 수정이다.
   **두 번째 실행 (run 36214396262 — ✗ db · kafka, 이번에는 원인이 보였다)**: ③ `chaos-db` 는 **콜드 스타트**였다 — 스택 기동 30초 뒤 첫 시나리오였고,
   주문 둘이 sim-runner 의 클라이언트 타임아웃(5초)에 걸렸다(max 5,079.7 ms · p95 77.6 ms — 웜 실행은 11 ms · 같은 시각 order-service 의 Redis 호출 실패).
   저장소는 이미 답을 갖고 있었다 — 콜드 스타트는 측정 조건이고(7-0 D1) 재기동 뒤에는 `make smoke` 로 데운다(RB-06 §3). 워크플로가 그 절차를 건너뛰었다 →
   예열 단계(비-201 은 판정이 아니고, 뒤의 검증 표가 판정). ④ `chaos-kafka` 는 **T0 의 초 내림**이었다 — 앞 시나리오가 03:23:37 에 끝나고 T0 도
   03:23:37 이라 그 초의 주문이 T0 뒤로 셌다(1203 ≠ 1200) → 기준은 한 초를 넘긴 뒤에 내린다. `redis` · `kill` 은 두 실행 모두 ✅.
   **세 번째 실행 (run 36215849539 — ✅ 넷 다)**: 예열에서 콜드 스타트가 **세 번째로 재현됐다**(200 중 198 · 응답 없음 2 · max 5,066.8 ms — 이번에는 판정이
   아니고, 뒤의 검증 표 ✅). `chaos-db` 재시도 0 → 36 · 나이 최대 360초 → 0 · `ConsumerLag` firing / `chaos-kafka` outbox 지연 297초 · 복구 뒤 0 /
   `chaos-redis` 지연 최대 0.112초 / `chaos-kill` 전제 5.7초 · 행 0 · 재기동 52초 뒤 `PUBLISHED`. GitHub 러너에서 처음으로 넷이 한 스택에서 초록이다.
...
4a. **peak-day 의 전제**(A27) — 시나리오 넷(`normal-day` · `peak-day` · `overload-day` · `cold-heavy`, 부록 A), 함대 변형
   ...
   **① 시뮬레이션 시계 ✅ (2026-09-26, [ADR-066](adr/ADR-066-simulation-moves-the-clock-not-the-schedule.md))** — 컷오프가 상수라 「직전 버스트」는
...
5. 런북 RB-01~04 · 06 · 07(RB-05 는 Phase 6 에 있다 — A22). **알림 14개 밖의 절차 하나 — 「조기 마감을 거듭한 날의 미제공」**
   (`MAX_PUSHES_EXCEEDED`, [ADR-063](adr/ADR-063-no-open-wave-is-not-a-late-event.md)). RB-06(피크 대비 체크리스트)에 **예열 항목**(D1), `docs/postmortems/2026-xx-peak-simulation.md`(가상 장애: 컷오프 시 계획 지연 → FAST 전환 → 원인·재발 방지, 실제 측정치 기반).
   **런북 ✅ (2026-09-25 — 7-3 · 7-4 보다 먼저, 사용자 결정)** `d27103c` · `caa6043` · `6400ebf` · `e355f72` · `e8290bd`: 표 하나(알림 14 × 대응 — 먼저 본다 · 갈래 · 대응 · 절차)에
   알림 밖 절차 셋(끊긴 트레이스 · `in_progress` 가 줄지 않음 · 조기 마감을 거듭한 날의 미제공), RB 여섯. 모든 명령을 로컬 스택에서 한 번 돌렸고,
   DB 장애는 재현했다(RB-02 §3). **7-3 이 RB-01 · 03 에 검증 SQL 을, 7-4 가 RB-06 §3 에 콜드 스타트 실측을 옮긴다.**
   **포스트모템은 7-4 뒤**(A23 — 실제 측정치가 입력이다).
   **7-5b** (2026-09-25, 7-5 리뷰가 연 작은 PR): 조기 마감은 그 캠프 · 티어의 가장 이른 열린 웨이브에만 — 409 `not-next-wave`
   ([ADR-054](adr/ADR-054-early-wave-close-is-an-operator-cutoff.md) 후속, ADR-063 재검토 지점의 순서 쪽을 닫는다) · §9.1 「있는 시계열이
   다른 것의 최솟값에 가려진다」 · OTLP 메트릭 내보내기를 끈다(매분의 소음).
