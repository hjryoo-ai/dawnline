...
8. **`small` 레짐 격차 — ✅ 닫혔다** (2026-09-09, [측정](benchmarks/phase4-local-search.md) §4).
   ...
   **17번이 들어갔다** (2026-09-12, [ADR-039](adr/ADR-039-reserve-seats-by-constraint-class.md)).
   그래서 **위 네 조합은 전부 옛 기준의 것**이다 — A 행의 자리에 지금 있는 값은
   1,136,026 / 3,893,515 / **8,281,646** 이고, 네 조합은 이 위에서 다시 재야 한다.
   `large` 의 1.65%p 를 만든 항(`priority-boost` 의 `÷ position`)은 17번에서도 그대로 나타났다
   (+31,336) — **같은 항이 두 항목을 연속으로 졌다**는 것이 18번의 근거를 한 번 더 준 셈이다.
