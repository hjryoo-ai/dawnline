...
9. **테스트 격리 — 픽스처가 정하지 않은 축이 넷 있다.** 공통점은 하나다: 지금 깨지지 않는
   이유가 *테스트에 적혀 있지 않다.*
...
10. **미배정 선택 규칙 — §6.5 3단계 정정** (2026-09-05 결정). 3단계의 "그래도 없으면 미배정" 은
    ...
    **우선도의 출처는 11번이 정한다** (2026-09-05 결정). 5a 에 적어 둔 대로
    `fulfillment.planned` 에 `priority` 가 없어 운영 경로의 후보는 전부 0 이고, 그러면 이 규칙은
    벤치마크에서만 구별을 만든다. 11번이 그 값을 만들어 준 뒤에 이 항목을 끝낸다.

11. **우선순위는 선언이 아니라 파생이다 — ADR-028** (2026-09-05 결정, 10번 착수 전에).

    계약에 `priority` 필드를 넣는 길은 두 가지로 막힌다. **클라이언트 값은 신뢰할 수 없고**
    (고객 API 는 무인증이다 — §10, `customerId` 조차 클라이언트 주장값이다), **티어에서
    파생하면 상수가 된다** — 웨이브가 (캠프, 티어, 컷오프) 단위라 한 계획 안의 모든 후보가 같은
    티어다. 아무것도 가르지 못하는 값은 우선순위가 아니다.

    한 계획 <em>안에서</em> 실제로 달라야 하는 것은 **"이 주문을 우리가 이미 얼마나 실망시켰는가"**
    이다. dispatch 가 후보 적재 시점에 이미 받은 사실에서 계산한다:

    | 사실 | 가중치 | 근거 |
    |---|---|---|
    | `promiseRevised` | **+2** | [ADR-020](adr/ADR-020-cutoff-ownership-wave-grace-promise-revision.md) 의 개정은 <em>한 번 깬 약속</em>이다 |
    | `requiresCold` | **+1** | 미배정의 비용이 다른 주문보다 크다(§6.3 cold-chain) |
    | 배송 실패 후 재배송 | **+3** | Phase 5 에서 채운다 — 그 사실은 tracking 이 만든다 |

    이걸로 10번의 "누구를 안 싣는가" 가 의미를 갖는다 — **약속을 이미 개정한 주문을 또 빼는 일이
    없어져야 한다.** `PRIORITY_BOOST`(§6.3)도 그때 처음 발화한다. 점수표는 룰처럼 **데이터로**
    두고, **계약 변경은 없다.**

    **범위 밖**: VIP 같은 고객 등급 우선순위. 고객 서비스가 없어 등급의 출처가 없고, 있는 척하면
    `serviceTier` 를 등급으로 몰래 읽는 코드가 된다. ADR 에 그렇게 적는다.

12. **`make demo` 와 CI 스모크가 하루 8시간 실패한다 — `[결정 필요]`** (2026-09-05 발견).
    ...
    **결정이 필요한 것은 그 다음이다.** 이 시드에는 **새벽 근무조가 없다** — 이 플랫폼의 이름이
    ...
    A 가 설계에 맞고 B 는 증상만 가린다. 다만 A 는 **부록 A 를 고치는 설계 변경**이라
    `CLAUDE.md` 대로 먼저 물어야 한다.

13. **계획 영속화 20초 — auto-flush 더티 체크. `[결정 필요]`** (2026-09-07 측정).
    ...
    | 안 | 내용 | 대가 |
    |---|---|---|
    | A | 저장 직전 `HibernateFlushMode.COMMIT` | flush 시점이 바뀌어 `markCandidates` 와 순서 의존이 생긴다 |
    | B | 라우트·설명 저장을 `JdbcTemplate` 배치로 (영속성 컨텍스트 밖) | 어댑터 하나를 EntityManager 에서 뗀다. 21.9배와 배치 이득이 함께 온다 |
    | C | 그대로 둔다 | 26.8초는 30초 예산 안이다 — 다만 여유가 **12%** 뿐이라 후보 6,000건에서 넘는다 |

    **§6.7 의 30초는 알고리즘 예산이다.** 지금은 그 예산의 75% 를 ORM 이 쓰고 있어서 "계획이
    예산 안에 끝났다" 는 통과가 최적화에 대해 말해 주는 것이 거의 없다. 1번
    LocalSearchImprover 는 계획 시간을 늘리는 작업이라 가장 먼저 여기에 부딪힌다 —
    **1번보다 먼저 결정해 두는 편이 낫다.**
