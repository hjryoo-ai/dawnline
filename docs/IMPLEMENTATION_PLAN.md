...
- `large`에서 `sweep-greedy-nn+ls`가 `baseline-nn` 대비 총비용 ≥ 15% 절감(미달 시 튜닝 기록과 실제 수치 보고).
  - **2026-09-09 현재 −7.57%** (12,964,445 / 14,026,147). 튜닝 기록과 분해는
    [`phase4-local-search.md`](benchmarks/phase4-local-search.md) §2~3. 부족분 1,042,220원이
    **개선 단계가 닿는 항 안에 없다** — 미배정 페널티가 총비용의 36.8%이고, 거리비를 0 으로 만들어도
    22만원이 모자란다. `medium` 은 **−16.82%** 로 이미 통과한다.
