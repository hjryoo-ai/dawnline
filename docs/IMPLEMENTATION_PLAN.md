...
8. **`small` 레짐 격차 — ✅ 닫혔다** (2026-09-09, [측정](benchmarks/phase4-local-search.md) §4).
   ...
   **판정은 18번 뒤로 미룬다.** 손해가 한 항뿐이고 그 항이 세 번째로 같은 자라면, 「C′ 을
   넣을까」는 **자가 바뀐 뒤에 물어야 하는 질문**이다. 순서: **18 → C 판정 → 2 → 5**.
...
18. **`priority-boost` 의 `÷ position` 은 §6.3 의 정책 문장을 정직하게 재는가 — 룰 설계 검토**
    ...
    **제안은 (b′)** — 보너스는 **계획 시작 대비 계획 도착 시각**으로 감쇠한다. 라우트 길이에
    구조적으로 중립이고(시간은 stop 이 몇 개인지 모른다), §2.2 의 「조기 배송은 허용·지각만
    벌한다」와 일관된다. **반론**: (b′) 는 약속과 무관하게 이른 것을 상 준다 — 창이 14시에 열리는
    고객을 6시에 배송하는 계획에 보너스를 준다. 그것이 정책으로 맞는지는 수치가 답할 수 없다.
