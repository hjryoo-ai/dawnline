...
| 8 | 릴레이 리더 락 + ADR-027 | ✅ | #33 `6eadc2b` |
| — | **`rules:camp:{id}:v{n}` 룰셋 캐시 (§7.2)** | ⬜ **미구현** | 넣지 않았다. 룰은 캠프당 10행 남짓이고 계획 한 번에 한 번 읽는다 — 데모 29계획에서 룰 조회가 29회다. 이 규모에서는 DB 조회가 맞다. 재검토 지점: 부분 재계획(§6.8)이 라우트마다 룰을 다시 읽게 되는 Phase 5 |
| — | **`lock:plan:{waveId}` 이중 안전장치 (§5.3, §7.2)** | ⬜ **미구현** | 중복 계획을 실제로 막는 것은 `route_plans.wave_id UNIQUE` 이고(5b), 그것은 DB 제약이라 인스턴스 수와 무관하다. Redis 락은 그 위의 *낭비 절감*이지 정확성이 아니다. 재검토 지점: dispatch 를 2개 이상으로 올릴 때 — 같은 웨이브가 두 인스턴스에서 동시에 <em>계획을 돌리고</em> 한쪽이 UNIQUE 로 버려지는 낭비가 보이면 |
