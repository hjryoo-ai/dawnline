...
8. **`small` 레짐 격차 — 조건부 항목** (Phase 3-4 게이트 결정의 후속). 1번의 국소 탐색이 들어간
   ...
   - **닫히지 않으면**: **크기 조건부 클러스터링** — 차량 수 ≤ k 면 스윕을 건너뛰고 최근접
     이웃으로 간다. 그때 ADR 을 쓴다. 이것이 허용되는 이유는 **메커니즘 기반 규칙**이기
     때문이다("자유도가 없는 레짐에서는 클러스터링이 값을 만들지 못한다"). 전략을 여러 개
     돌려 보고 이긴 것을 고르는 **best-of-N 메타 전략은 게이트를 통과시키는 방법이 아니다** —
     그것은 게이트가 재려는 것을 재지 못하게 만든다.
