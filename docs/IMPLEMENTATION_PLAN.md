...
14. **통합이 제약을 전파한다 — [결정 필요]** (2026-09-09 발견, 10·11 측정의 부산물).

    §6.5 1단계의 통합 키는 `geohash7 + 약속창` 이다. 같은 건물·같은 창의 주문 열 건 중 **하나가
    위험물이면 stop 전체가 위험물**이 되고, 열 건이 함께 위험물 차량을 기다린다. `large` 의
    미배정 89건이 실제 제약 주문 수보다 훨씬 많은 이유가 이것이다(stop 18개 × 평균 4.9건).

    | 안 | 성격 | 대가 |
    |---|---|---|
    | A. 통합 키에 `(requiresCold, hazmat)` 추가 | **알고리즘** — §6.5 1단계 설계 변경 | stop 수 증가. 같은 좌표 사이 이동은 0 이라 거리비는 늘지 않는다. 재기준 필요 |
    | B. `DatasetFeasibilityTest` 에 「제약 조합별 수요 ≤ 그 조합 차량 용량의 70%」 기준 추가 | **측정 기준** — §6.9 | 지금 데이터셋이 **실패한다**. 고치면 재기준 필요 |
    | C. 그대로 두기 | — | `large` DoD 가 −11.84% 에서 멈추고, 그 원인이 문서에만 남는다 |

    A 와 B 는 배타적이지 않다 — **B 가 기준이고 A 가 알고리즘**이다. 기준을 수치보다 먼저 적는
    규칙(§6.9)대로면 B 가 먼저다. 둘 다 §6.9 동결 규칙의 **재기준 기록** 대상이다.

**DoD**
- `large`에서 `sweep-greedy-nn+ls`가 `baseline-nn` 대비 총비용 ≥ 15% 절감(미달 시 튜닝 기록과 실제 수치 보고).
  - **2026-09-09 현재 −11.84%** (12,366,055 / 14,026,147). 국소 탐색이 −7.57%, 미배정 재삽입이
    −11.84% 까지 옮겼다. 튜닝 기록은 [`phase4-local-search.md`](benchmarks/phase4-local-search.md) §3,
    남은 격차의 원인 분해는 [`phase4-unassigned-policy.md`](benchmarks/phase4-unassigned-policy.md) §3.
    **부족분은 알고리즘이 닿는 곳에 없다** — 남은 미배정 18 stop 전부가 실행 가능성으로 막혀 있고
    (예산이 막은 것 0건), 원인은 데이터셋의 제약 조합 부족과 통합의 제약 전파다(위 14번).
    `medium` 은 **−17.27%** 로 이미 통과한다.
- 5,000 주문 계획 p95 ≤ 30초(미달 시 프로파일링 결과·병목 문서화).
  - **통과 — 3,247 ms** (여유 9배). Phase 3 대비 탐욕 단계가 15배 빨라진 것이 함께 들어 있다.
