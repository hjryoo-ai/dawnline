...
2. **`savings-cw+ls` 전략 — ✅ 완료** (2026-09-17,
   ...
   **그런데 기본 전략은 바꾸지 않았다.** `peak` 에서 30초 예산에 잘리고 **잘린 값이 실행마다
   ...
   96 stop 이 `plan-deadline` 로 남았고 기본보다 +1,767,471원). 바꾸는 조건은 미리 적었다 —
   「`peak` 30초 수렴」이고 부족분은 **3초**, 그 시간의 **97%가 재삽입**이다(구성 241 ms ·
   부착 170 ms · CW 라우트 216개 중 128개가 밀려 3,880 stop 이 재삽입으로 내려간다).
...
- `large`에서 `sweep-greedy-nn+ls`가 `baseline-nn` 대비 총비용 ≥ 15% 절감(미달 시 튜닝 기록과 실제 수치 보고).
  ...
  - **2026-09-17 — 넘었다. −19.30%. 다만 그것을 낸 전략은 기본 전략이 아니다.**
