...
3. **병렬화 — ⏸ 게이트 뒤로 미뤘다** (2026-09-11, [ADR-035](adr/ADR-035-parallel-unit-is-not-the-cluster.md) · [측정](benchmarks/phase4-where-the-time-is.md)).
   ...
   **순서는 「안 훑기(15번) → 게이트(`peak`) → 병렬」이다.** 게이트: 15번 적용 후 `peak`
   (15,000/60)를 순차로 재서 30초 예산이 실제로 잘라 내는지, 잘림의 대가가 120초 실행 대비
   얼마인지 본다. **1% 미만이면 병렬화는 이월**하고 그 이월을 ADR-008 이 수치와 함께 기록한다.
   게이트를 넘으면 위 표의 배정 팬아웃 + 라우트별 2-opt·Or-opt 부터(둘 다 동일성이 구조로
   보장되는 자리), 라우트 사이의 투기적 병렬은 15번 뒤에 **남은 시간을 보고** 무게를 정한다.
...
15. **안 훑기 — 라우트 사이 스캔의 97.6%가 헛스캔이다** (2026-09-11 측정, [ADR-035](adr/ADR-035-parallel-unit-is-not-the-cluster.md) 5번 · [측정](benchmarks/phase4-where-the-time-is.md)).
    ...
    **조건은 정확(exact)이다.** 결과가 전체 스캔과 **한 자리도 달라선 안 되고**, 그것을
    ...
    3번(병렬화)의 게이트가 이 항목 뒤에 있다. 안 훑기가 라우트 사이를 절반 이하로 줄이면 `peak`
    도 수렴할 수 있고, 그러면 병렬화는 「만들 수 있지만 만들 이유가 측정되지 않은」 항목으로 남는다.
