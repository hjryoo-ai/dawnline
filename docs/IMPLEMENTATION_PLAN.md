...
### 16-c. 재삽입의 정확한 가지치기 — ⏳
    ...
    마감 전에 낭비를 줄인다. stop 의 속성 클래스에 대해 **남은 용량(stop 수·중량·부피)이 0인
    라우트는 시도하지 않고**, 전 라우트가 0이면 그 클래스의 미배정은 통째로 건너뛴다.
    **결과가 바뀔 수 없는 규칙**(시도해도 못 들어가는 자리를 안 시도하는 것)이라 동일성
    어설션으로 지킨다. `overload` 의 26초 대부분이 여기서 빠질 것으로 **예상**하지만 예상은
    수치가 아니다 — 재서 확인한다. 그래도 남으면 k-최근접 라우트 한정은 휴리스틱이라
    재기준 경로로 별도 커밋.
