...
- `large`에서 `sweep-greedy-nn+ls`가 `baseline-nn` 대비 총비용 ≥ 15% 절감(미달 시 튜닝 기록과 실제 수치 보고).
  - **2026-09-09 현재 −14.53%** (8,276,130 / 9,682,478). **두 번 재기준된 수치다** —
    −11.84%(측정 도구가 용량 부족을 재던 상태) → −43.98%(도구를 고쳤으나 자가 반대로 휜 상태)
    → **−14.53%**(모델까지 고친 상태). 세 열은
    [`phase4-constraint-classes.md`](benchmarks/phase4-constraint-classes.md) §4.
    `medium` 은 **−15.47%** 로 통과한다. 튜닝 기록은
...
17. **희소 능력 좌석 — 위험물 차량이 일반 수요로 먼저 채워진다** (2026-09-12 8번이 열었다,
    [ADR-038](adr/ADR-038-fixed-cost-floor-is-not-a-total-cost-floor.md) ·
    [측정](benchmarks/phase4-cluster-axis.md) §5. **[결정 필요]**).
    ...
    **§6.9 규칙대로 그림자 계측이 먼저다.** 재는 것 셋: ⓐ 희소 능력 차량(위험물·냉장)에 실린
    **일반 stop 수**와 그 좌석을 비웠을 때 회수되는 미배정 페널티의 **상한**, ⓑ 그 상한이
    배정 순서(희소 수요를 먼저)로 걷히는가 좌석 예약(용량의 일부를 남긴다)으로 걷히는가,
    ⓒ `leftover` 첫째 키를 그대로 둔 채로도 걷히는가 — 셋째 키를 첫째로 올리는 것은
    **휴리스틱 변경이라 재기준 경로**다.
    ...
    **B(설계대로 배정)도 단독으로 넣지 않는다.** B 가 `large` 에서 −3.83% 였던 것은 구현의
    실패가 아니라 **§6.5 3단계 문장의 불완전**이다 — 그 사실은 설계서에 지금 표시했다(「좌석
    예약 없이는 불완전, 4-17 에서 정정 예정」). 이 항목이 그 정정이다.

    > 사용자가 이 항목을 **4-16** 으로 불렀는데 16번은 이미 「`peak` 이 드러낸 셋」이 쓰고 있어
    > **17번**으로 둔다. 15번이 같은 이유로 한 칸 밀린 것과 같다.

18. **`priority-boost` 의 `÷ position` 은 의도된 정책인가 부수 효과인가 — 룰 설계 검토**
    (2026-09-12 8번이 열었다, [ADR-038](adr/ADR-038-fixed-cost-floor-is-not-a-total-cost-floor.md)
    §4 · [측정](benchmarks/phase4-cluster-axis.md) §4. **[결정 필요]** · **Phase 4 마감 뒤**).

    소프트 룰 `PRIORITY_BOOST` 의 값은 `bonusKrw × priority ÷ position` 이다. `÷ position` 은
    라우트가 길어질수록 보너스가 줄어들므로 **구조적으로 「차를 더 쓰는」 계획에 유리한 자**다.
    측정: 축 정정(8번)이 `large` 에서 우선 stop 을 **한 건도 잃지 않고**(477건 그대로) 자리만
    뒤로 밀었는데 그것만으로 **+83,923원**이었다. 라우트 수를 줄이는 **모든** 작업이 이 항에서
    손해를 본다.

    질문은 최적화가 아니라 **정책**이다: 「우선 고객은 짧은 라우트로 빨리」가 뜻한 것인가,
    아니면 「앞 순서에」를 위치로 나눠 적었더니 따라온 부수 효과인가. 전자면 지금이 맞고 —
    고정비를 줄이는 항목이 이 항을 함께 보고하면 된다 — 후자면 감쇠를 위치가 아니라 **예상 도착
    시각**에 걸어야 한다(그래야 「빨리」가 라우트 길이가 아니라 시간을 뜻한다).

    **지금 손대지 않는다.** 최적화가 불리해진다는 이유로 목적함수를 고치는 것은 **재는 자를
    고치는 일**이고, 그러면 Phase 4 의 모든 수치가 «자를 바꿔서 좋아진 것» 이 된다. 여는 조건은
    셋이다: ① **Phase 4 마감 뒤**, ② **별도 ADR** 로만, ③ **전 데이터셋 재기준을 동반**한다
    (동결 베이스라인도 다시 낸다 — 자가 바뀌면 §6.9 게이트 규칙 1 의 「동결」이 다른 자의 값이다).
