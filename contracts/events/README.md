...
| `dawnline.delivery.route-departed.v1` | O | Phase 6 묶음 B (ops-api — **소비자 주도**, [ADR-050](../../docs/adr/ADR-050-route-departure-is-an-event.md)), 발행 tracking (2026-09-24) |
...
| `STALE_PLACED` | FC 선택 **전**. `cutoffAt < now − 24h` — 지각 도착 흡수 경로의 상한(ADR-020 후속 정정) |
| `MAX_PUSHES_EXCEEDED` | FC 선택 **뒤**. 원래 컷오프의 웨이브와 밀린 셋이 **모두 닫혀 있다** — 받을 웨이브가 없다(ADR-063) |

`STALE_PLACED` 는 나머지와 성격이 다르다. 다른 사유는 "이 주소·상품을 지금 배송할 수 없다" 인데,
이것은 **"이 이벤트가 너무 늦게 왔다"** 이다. 20일 묵은 `order.placed` 가 DLQ replay 로 들어와도
다음 웨이브로 밀려 새 배송 약속이 나가지 않게 하는 방어이며, 그 주문을 살릴지는 사람이 정한다
(RB-05 §2.2).

`MAX_PUSHES_EXCEEDED` 는 `STALE_PLACED` 와 **갈라 둔다**. 둘 다 「웨이브에 넣지 못했다」지만 원인이 반대편에 있다 —
`STALE_PLACED` 는 이벤트가 늦게 왔고, 이것은 이벤트는 제때 왔는데 **같은 캠프 · 티어의 웨이브가 연달아 닫혀 있다**.
그렇게 되는 길은 사실상 운영자의 조기 마감을 거듭한 것뿐이다(ADR-054). 2026-09-25 까지 이 경우도 `STALE_PLACED` 로
나갔고, 그래서 운영자는 그날 자기가 누른 마감의 결과를 「늦게 온 주문」으로 읽었다(ADR-063).
...
**예외 — 아직 한 번도 발행된 적 없는 스키마.** 첫 항목("`required` 에 넣지 말 것")의 근거는
"기존 발행자는 그 필드를 보내지 않는다" 이다. 그런 발행자가 존재한 적이 없다면 근거도 없고,
필수여야 할 필드를 선택으로 두면 소비자가 "없을 때" 를 처리하는 죽은 분기를 갖게 된다.
아래 셋을 **모두** 만족할 때만 `required` 에 넣고, 그 사실을 여기 남긴다.

1. 그 `eventType` 의 토픽이 어떤 환경에도 만들어진 적이 없다
2. 발행자 코드가 배포된 적이 없다
3. 소비자가 아직 없거나, 같은 변경에서 함께 고친다
...
| 2026-09-18 | `route.assigned.v1` 의 stop 에 `promisedWindow` 추가 (required) | Phase 5-1a. 발행자(dispatch-service)는 **배포된 적이 없고 같은 변경에서 함께 고친다** — 조건 2·3 이 그대로다. 운영 소비자는 0 이다: tracking-service 는 **지금 만드는 중**이고 ops 는 Phase 6 다. 조건 1(토픽이 만들어진 적 없음)만 `wave.closed` 때와 같은 예외를 탄다 — 로컬 개발 볼륨에는 있으나 어떤 배포 환경에도 없고, 그 볼륨은 재생성된다. 선택으로 두면 소비자가 「없을 때」 분기를 갖는데 **그 분기의 답이 없다**: tracking 의 핵심 동작이 at-risk 판정(`eta > promised_end − 15분`, §5.4)이고, 약속창이 없으면 그 stop 은 판정에서 빠진다 — 카운터로 시끄럽게 만들어도 그것은 핵심 기능의 조용한 열화다. 값의 출처는 dispatch 가 이미 갖고 있다(`StopMerger` 의 병합 키가 「같은 약속창」이라 stop 당 창이 하나로 정해진다, §6.5 1단계). **운영 메모**: 개발 볼륨의 Kafka 에 남은 이전 `route.assigned` 이벤트에는 이 필드가 없다. tracking 의 컨슈머 그룹은 `latest` 에서 시작하거나 그 토픽을 재생성한다 — 적지 않으면 첫 기동에서 DLQ 가 찬다 |
| 2026-09-19 | `route.assigned.v1` 의 `summary` 에 `plannedDeparture` 추가 (required) | Phase 5-1b. 조건 셋이 하루 전과 같다 — 발행자(dispatch-service)는 배포된 적이 없고 **같은 변경에서 함께 고친다**(조건 2·3), 운영 소비자는 0 이다(tracking 은 아직 머지 전, ops 는 Phase 6). 조건 1 은 `promisedWindow` 와 같은 예외를 탄다 — 로컬 개발 볼륨에만 있고 그 볼륨은 이미 재생성 대상이다. **선택으로 둘 수 없는 이유**: 늦은 출발이 가장 흔한 지연 원인이고 그것은 첫 `ARRIVED` 스캔 <em>전에</em> 이미 알 수 있다. 값이 없으면 tracking 은 첫 도착까지 아무 위험도 감지하지 못하고, 그 열화는 조용하다 — 「창이 없는 stop 은 판정에서 빠진다」와 같은 모양이다. 값의 출처는 dispatch 가 이미 갖고 있었다: `RouteState` 의 출발 앵커(근무창 시작과 계획 시작 중 늦은 쪽, §6.3 · ADR-030)이고, `PlannedRoute` 로 굳히면서 버려지고 있었다. 저장은 `routes.planned_departure`(V7, NULL 허용 — V6 과 같은 규칙: V7 이전 행에는 참인 값이 없고 지어내는 대신 발행이 소리 내어 실패한다) |
