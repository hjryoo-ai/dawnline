...
**벤치마크** ([`docs/benchmarks/phase4-priority-boost.md`](docs/benchmarks/phase4-priority-boost.md) §8, seed 고정, 커밋 단위 측정):

| 데이터셋 | `baseline-nn` | `sweep-greedy-nn` | `sweep-greedy-nn+ls` (기본) | 계획 p95 |
|---|---:|---:|---:|---:|
| small (500주문 / 5대) | 1,517,523 | 1,313,886 (−13.4%) | **1,120,725 (−26.15%)** | 342 ms |
| medium (2,000 / 20) | 4,588,065 | 4,236,979 (−7.7%) | **3,848,378 (−16.12%)** | 1,673 ms |
| large (5,000 / 40) | 9,577,578 | 8,979,466 (−6.2%) | **8,182,826 (−14.56%)** | 4,260 ms |

> **이 표는 2026-09-12 에 재기준됐다** ([ADR-040](docs/adr/ADR-040-priority-boost-decays-in-time.md)):
...
> 미배정은 **0** 이다(자를 바꾼 뒤로는 −13.4% · **−26.15%**, ADR-040). 다섯째 차는 여전히
> 있다 — 그 예측만은 처음부터 맞았다.
> CI 회귀 게이트는 자유도가 처음 생기는 `medium` 에서 돈다(§6.9, 결과가 아니라 메커니즘이 기준이다).

> **`large` 의 「≥ 15% 절감」 목표는 −14.56% 로 아직 미달이다 — 41,885원 부족하고, 이 숫자는 네 번 재기준된 것이다.** 처음 측정은 −11.84% 였는데, 총비용의 **34%가 누구도 실을 수 없는 미배정 페널티**였다(냉장 ∧ 위험물 차량이 데이터셋 크기와 무관하게 1대). 도구를 고치자
...
> 바꾸자 `large` 의 기본 전략 주행거리가 **58 km 줄었다**. 그래도 **−14.56%** 다 —
> **판단 기준에서 「어느 정의가 15%를 넘기는가」는 뺐고**, 그 문장을 ADR 본문에 박아 두었다.
> `p95 ≤ 30초` 는 **4,260 ms 로 통과**한다.
