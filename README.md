...
|---|---:|---:|---:|---:|
| small (500주문 / 5대) | 1,510,366 | 1,642,762 (+8.8%) | **1,451,439 (−3.90%)** | 251 ms |
| medium (2,000 / 20) | 5,032,092 | 4,581,241 (−9.0%) | **4,185,463 (−16.82%)** | 1,404 ms |
| large (5,000 / 40) | 14,026,147 | 13,610,957 (−3.0%) | **12,964,445 (−7.57%)** | 3,247 ms |
...
> **`large` 의 「≥ 15% 절감」 목표는 아직 −7.57% 다.** 원인은 라우팅 품질이 아니라 **미배정**이다 —
> 총비용의 **36.8%(4,770,000원)** 가 미배정 페널티이고 개선 단계는 그 항을 보지 못한다.
> 거리비를 0 으로 만들어도 목표에 22만원이 모자란다. 미배정 99건은 전부 위험물이고, 다음 결정은
> 「미배정 stop 을 라우트에 끼워 넣는 이동을 §6.5 5단계에 넣을 것인가」다
> ([분해](docs/benchmarks/phase4-local-search.md) §2). `p95 ≤ 30초` 는 **3,247 ms 로 통과**한다.
