...
**벤치마크** ([`docs/benchmarks/phase4-cluster-axis.md`](docs/benchmarks/phase4-cluster-axis.md) §9, seed 고정, 커밋 단위 측정):

| 데이터셋 | `baseline-nn` | `sweep-greedy-nn` | `sweep-greedy-nn+ls` (기본) | 계획 p95 |
|---|---:|---:|---:|---:|
| small (500주문 / 5대) | 1,517,523 | 1,277,725 (−15.8%) | **1,113,911 (−26.60%)** | 232 ms |
| medium (2,000 / 20) | 4,588,065 | 4,285,740 (−6.6%) | **3,791,148 (−17.37%)** | 1,271 ms |
| large (5,000 / 40) | 9,577,578 | 9,212,551 (−3.8%) | **8,147,294 (−14.93%)** | 3,303 ms |
...
> **`large` 의 「≥ 15% 절감」 목표는 −14.93% 로 아직 미달이다 — 6,353원 부족하고, 이 숫자는 다섯 번 재기준된 것이다.** 처음 측정은 −11.84% 였는데, 총비용의 **34%가 누구도 실을 수 없는 미배정 페널티**였다(냉장 ∧ 위험물 차량이 데이터셋 크기와 무관하게 1대). 도구를 고치자
