...
**벤치마크** ([`docs/benchmarks/phase4-constraint-classes.md`](docs/benchmarks/phase4-constraint-classes.md), seed 고정, 커밋 단위 측정):

| 데이터셋 | `baseline-nn` | `sweep-greedy-nn` | `sweep-greedy-nn+ls` (기본) | 계획 p95 |
|---|---:|---:|---:|---:|
| small (500주문 / 5대) | 1,531,480 | 1,624,788 (+6.1%) | **1,490,513 (−2.68%)** | 255 ms |
| medium (2,000 / 20) | 4,636,549 | 4,290,728 (−7.5%) | **3,919,106 (−15.47%)** | 1,248 ms |
| large (5,000 / 40) | 9,682,478 | 9,053,096 (−6.5%) | **8,276,130 (−14.53%)** | 5,829 ms |

> **`small` 레짐 격차는 닫혔다.** 클러스터링 단독(`sweep-greedy-nn`)은 `small` 에서 베이스라인에
> +8.8% 로 **졌다** — 차량이 4~5대면 "누구를 어느 차에 태울지" 의 자유도가 사실상 없어(최소 필요
> 차량 4대) 다섯째 차의 고정비 45,000원이 순수한 손실이 되기 때문이다. 개선 단계가 붙은 뒤
> **−2.68%** 로 뒤집혔다. 다섯째 차는 여전히 있고 — 그 예측은 맞았다 — 거리·시간·지각에서 넘겼다.
> 여유는 얇다: 통합 키에 제약을 넣으면서(ADR-033) `small` 은 −3.90% → −2.68% 로 되밀렸다 —
> 차 5대 규모에서는 통합 분할이 stop 수만 늘린다.
> CI 회귀 게이트는 자유도가 처음 생기는 `medium` 에서 돈다(§6.9, 결과가 아니라 메커니즘이 기준이다).

> **`large` 의 「≥ 15% 절감」 목표는 −14.53% 로 아직 미달이다 — 그리고 이 숫자는 두 번 재기준된 것이다.** 처음 측정은 −11.84% 였는데, 총비용의 **34%가 누구도 실을 수 없는 미배정 페널티**였다(냉장 ∧ 위험물 차량이 데이터셋 크기와 무관하게 1대). 도구를 고치자
...
> 예산을 다 썼다」이므로 다음 계획의 **개선 예산을 절반으로**(대가 **+1.35%**, `large` p95
> 4,898 → 3,208 ms), **파티션 컨슈머 랙 > 3 웨이브**는 「처리량이 모자란다」이므로 §6.5 5단계를
> 아예 **끈다**(FAST — 대가 **+9.39%**, 1,542 ms). 전략을 바꾸는 것이 아니다.
