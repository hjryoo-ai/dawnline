...
**벤치마크** ([`docs/benchmarks/phase4-savings-cw.md`](docs/benchmarks/phase4-savings-cw.md), seed 고정, 커밋 단위 측정):

| 데이터셋 | `baseline-nn` | `sweep-greedy-nn` | `sweep-greedy-nn+ls` (기본) | `savings-cw+ls` | 계획 p95 (기본 / savings) |
|---|---:|---:|---:|---:|---:|
| small (500주문 / 5대) | 1,517,523 | 1,277,725 (−15.80%) | 1,113,911 (−26.60%) | **1,073,363 (−29.27%)** | 224 / 231 ms |
| medium (2,000 / 20) | 4,588,065 | 4,285,740 (−6.59%) | 3,791,148 (−17.37%) | **3,705,570 (−19.24%)** | 1,297 / 1,076 ms |
| large (5,000 / 40) | 9,577,578 | 9,212,551 (−3.81%) | 8,147,294 (−14.93%) | **7,729,298 (−19.30%)** | 3,386 / 4,654 ms |

> **§6.7 의 「≥ 15% 절감」은 `large` 에서 −19.30% 로 넘었다 — 다만 그것을 낸 전략은 기본 전략이
> 아니다** ([ADR-042](docs/adr/ADR-042-savings-merges-are-class-aware.md),
> [ADR-043](docs/adr/ADR-043-default-strategy-stays-until-peak-converges.md)). Clarke-Wright
...
> 라우트를 고칠 수 없기 때문이다.
>
> **그런데 `peak` 에서 예산에 잘리고, 잘린 값이 실행마다 다르다** — 같은 커밋·seed·예산으로
> 20,940,782 / 21,173,960 / 20,940,782 / **23,277,318** 이다. 마지막은 마감이 개선이 아니라
> **배정을 끊어** 96 stop 이 `plan-deadline` 로 남은 실행이고, 현재 기본 전략보다 1,767,471원
> 비싸다. **마감이 무엇을 끊는지가 총비용이 얼마나 나쁜지를 정한다** — 개선을 끊으면 계획은 덜
> 좋을 뿐이지만 배정을 끊으면 계획에 구멍이 남는다. 그래서 기본 전략은 **바꾸지 않았고, 바꾸는
> 조건을 먼저 적었다** — 벽시계가 아니라 **구조**로: 「구성이 만든 라우트 수 ≤ 차량 수 × 1.2」
> (`peak` 에서 106, 지금은 **216**). 예산을 넘기는 것은 라우트 216개 중 128개가 부착에서 밀려
> 내려간 **3,880 stop 의 재삽입**이고(부족분 3초의 97%), 라우트 수는 기계가 바뀌어도 같다.
...
> 바뀐 것이다. 목표를 겨냥한 변경이 아니었다는 것이 이 항목의 기록이고, 기본 전략이 되지 못한
> 이유(`peak` 에서 잘린다)는 위 벤치마크 문단에 있다.
...
> 「처리량이 모자란다」이므로 §6.5 5단계를 아예 **끈다**(FAST — 대가 **+9.35%**, `large` p95
> 1,747 ms). 전략을 바꾸는 것이 아니다.
> ⚠️ **첫 단의 대가는 다시 재야 한다**: 좌석 예약(ADR-039)이 개선 단계의 일을 줄여서 `large` 의
> 「예산 절반」이 **더 이상 물지 않는다**(+1.35% → **+0.00%**, 절반 예산 안에 수렴). 사다리의
> 근거는 그대로지만 수치는 4-5 에서 전 데이터셋으로 다시 낸다.
> 처음에는 둘 다 FAST 를 냈는데 **45배 비싼 처방**이었고(+0.21% 대 +9.4%), 그것을 뒤집은 것은
> 우리가 낸 표였다. §6.7 의 「fast mode ≤ 5초」는 1,542 ms 로 통과한다.
