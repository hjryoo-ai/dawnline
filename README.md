...
**벤치마크** ([`docs/benchmarks/phase4-scarce-seats.md`](docs/benchmarks/phase4-scarce-seats.md) §8, seed 고정, 커밋 단위 측정):

| 데이터셋 | `baseline-nn` | `sweep-greedy-nn` | `sweep-greedy-nn+ls` (기본) | 계획 p95 |
|---|---:|---:|---:|---:|
| small (500주문 / 5대) | 1,531,480 | 1,287,456 (−15.9%) | **1,136,026 (−25.82%)** | 311 ms |
| medium (2,000 / 20) | 4,636,549 | 4,315,281 (−6.9%) | **3,893,515 (−16.03%)** | 1,545 ms |
| large (5,000 / 40) | 9,682,478 | 9,056,167 (−6.5%) | **8,281,646 (−14.47%)** | 4,574 ms |
...
> 미배정은 **0** 이다. 다섯째 차는 여전히 있다 — 그 예측만은 처음부터 맞았다.
> CI 회귀 게이트는 자유도가 처음 생기는 `medium` 에서 돈다(§6.9, 결과가 아니라 메커니즘이 기준이다).

> **`large` 의 「≥ 15% 절감」 목표는 −14.47% 로 아직 미달이다 — 51,540원 부족하고, 이 숫자는 세 번 재기준된 것이다.** 처음 측정은 −11.84% 였는데, 총비용의 **34%가 누구도 실을 수 없는 미배정 페널티**였다(냉장 ∧ 위험물 차량이 데이터셋 크기와 무관하게 1대). 도구를 고치자
...
> 볼 것은 알고리즘이 아니라 자다** — 그 검토는 Phase 4 마감 뒤 별도 ADR 로 연다(4-18).
> `p95 ≤ 30초` 는 **4,574 ms 로 통과**한다.
