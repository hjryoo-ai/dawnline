...
**벤치마크 — 개선 단계까지** ([`docs/benchmarks/phase4-local-search.md`](docs/benchmarks/phase4-local-search.md), seed 고정, 커밋 단위 측정):

| 데이터셋 | `baseline-nn` | `sweep-greedy-nn` | `sweep-greedy-nn+ls` (기본) | 계획 p95 |
|---|---:|---:|---:|---:|
| small (500주문 / 5대) | 1,510,366 | 1,642,762 (+8.8%) | **1,451,439 (−3.90%)** | 259 ms |
| medium (2,000 / 20) | 5,032,092 | 4,553,620 (−9.5%) | **4,162,954 (−17.27%)** | 1,350 ms |
| large (5,000 / 40) | 14,026,147 | 13,038,257 (−7.0%) | **12,366,055 (−11.84%)** | 4,002 ms |
...
> **−3.90%** 로 뒤집혔다. 다섯째 차는 여전히 있고 — 그 예측은 맞았다 — 거리·시간·지각에서 넘겼다.
> CI 회귀 게이트는 자유도가 처음 생기는 `medium` 에서 돈다(§6.9, 결과가 아니라 메커니즘이 기준이다).

> **`large` 의 「≥ 15% 절감」 목표는 아직 −11.84% 다 — 그리고 남은 격차는 알고리즘이 아니다.**
> 미배정 재삽입([ADR-028](docs/adr/ADR-028-unassigned-policy.md))이 −7.57% → −11.84% 로 옮겼고,
> 남은 미배정 stop 18개를 **전부** 조사한 결과 「실을 수는 있는데 값어치가 없어서 안 실은」 것은
> **0건**이다. 18개 전부 `냉장 ∧ 위험물` 이고 어느 라우트 어느 자리에도 들어가지 않는다 —
> 데이터셋에 그 조합을 실을 차가 **한 대뿐**이고, §6.5 1단계의 통합이 제약을 전파해 stop 하나가
> 평균 4.9건을 데리고 미배정이 되기 때문이다
> ([분해](docs/benchmarks/phase4-unassigned-policy.md) §3). `p95 ≤ 30초` 는 **4,002 ms 로 통과**한다.
